// Complete curated Durga Puja tracks, Mahalaya segments & Mondol Barir Pujo Heritage Data
// All video IDs and covers verified 100% active with authentic high-resolution artwork

export const playlists = {
  durgaPuja: {
    key: "durgaPuja",
    label: "সেরা পুজো ও আগমনী গান",
    pillLabel: "PUJA RADIO",
    description: "শারদোৎসবের শ্রেষ্ঠ ও জনপ্রিয় আগমনী গান, মহিষাসুরমর্দিনী ও সাবেকি পুজোর চিরসবুজ সুরমালা।",
    sourceType: "youtube",
    tracksAreDistinctVideos: true,
    tracks: [
      {
        id: "durga-001",
        num: "01",
        title: "দুগ্গা এলো (Dugga Elo)",
        title_bn: "দুগ্গা এলো",
        title_en: "Dugga Elo",
        artist: "Monali Thakur (মোনালি ঠাকুর)",
        composer: "Guddu",
        lyricist: "Indranil Das",
        videoId: "xlElO06nQy8",
        start: 0,
        end: 236,
        duration: 236,
        durationLabel: "3:56",
        cover: "https://img.youtube.com/vi/xlElO06nQy8/mqdefault.jpg",
        sourceUrl: "https://www.youtube.com/watch?v=xlElO06nQy8",
        description: "A joyful festive Agomoni song celebrating Maa Durga's auspicious arrival.",
        bengaliDesc: "মোনালি ঠাকুরের প্রাণবন্ত কণ্ঠে পুজোয় মা দুর্গার ঘরে ফেরার আনন্দ ও উচ্ছ্বাস।"
      },
      {
        id: "durga-ambient-00",
        num: "০২",
        title: "মন্ডল বাড়ি শারদ আবহ সঙ্গীত (Mondol Bari Ambient)",
        title_bn: "মন্ডল বাড়ি শারদ আবহ সঙ্গীত",
        title_en: "Mondol Bari Festive Ambient",
        artist: "ফুরফুরা মণ্ডল পরিবার আবহ",
        composer: "ঐতিহ্য শারদ সুরমালা",
        lyricist: "শারদোৎসব",
        videoId: "DZ21CSg22nc",
        start: 0,
        end: 245,
        duration: 245,
        durationLabel: "4:05",
        cover: "https://img.youtube.com/vi/DZ21CSg22nc/mqdefault.jpg",
        sourceUrl: "https://youtu.be/DZ21CSg22nc",
        description: "Serene sacred background ambient Durga Puja melody playing continuously in low tone.",
        bengaliDesc: "মন্ডল বাড়ির পুজো প্রাঙ্গণের জন্য নির্ধারিত শান্ত, স্নিগ্ধ ও আবহ সঙ্গীত।"
      },
      {
        id: "durga-002",
        num: "02",
        title: "দুগ্গা মা (Dugga Ma)",
        title_bn: "দুগ্গা মা (বলো দুর্গা মাইকি)",
        title_en: "Dugga Ma (Bolo Dugga Maiki)",
        artist: "Arijit Singh (অরিজিৎ সিং)",
        composer: "Arindom",
        lyricist: "Priyo Chattopadhyay",
        videoId: "sPuZ0Q3KDWo",
        start: 0,
        end: 271,
        duration: 271,
        durationLabel: "4:31",
        cover: "https://img.youtube.com/vi/sPuZ0Q3KDWo/mqdefault.jpg",
        sourceUrl: "https://www.youtube.com/watch?v=sPuZ0Q3KDWo",
        description: "Emotional homecoming Puja song capturing childhood memories and festive vibes.",
        bengaliDesc: "অরিজিৎ সিংয়ের মিষ্টি কণ্ঠ ও ঘরে ফেরার নস্টালজিক পুজোর অনুভূতি।"
      },
      {
        id: "durga-003",
        num: "03",
        title: "বলো দুগ্গা এলো (Bolo Dugga Elo)",
        title_bn: "বলো দুগ্গা এলো (দুগ্গা দুগ্গা)",
        title_en: "Bolo Dugga Elo (Dugga Dugga)",
        artist: "Sunidhi Chauhan (সুনিধি চৌহান)",
        composer: "Kaushik-Guddu",
        lyricist: "Indranil Das",
        videoId: "XyatKcoBrPw",
        start: 0,
        end: 200,
        duration: 200,
        durationLabel: "3:20",
        cover: "https://img.youtube.com/vi/XyatKcoBrPw/mqdefault.jpg",
        sourceUrl: "https://www.youtube.com/watch?v=XyatKcoBrPw",
        description: "High-energy Dhak rhythms and festive groove welcoming Devi Durga.",
        bengaliDesc: "সুনিধি চৌহানের ধামাকা কণ্ঠে ঢাকের তালের জমজমাট পুজোর গান।"
      },
      {
        id: "durga-004",
        num: "04",
        title: "ঢাক বাজা কাঁসর বাজা (Dhak Baja Kashor Baja)",
        title_bn: "ঢাক বাজা কাঁসর বাজা",
        title_en: "Dhak Baja Kashor Baja",
        artist: "Shreya Ghoshal (শ্রেয়া ঘোষাল)",
        composer: "Jeet Gannguli",
        lyricist: "Priyo Chatterjee",
        videoId: "id5_3dKvEBg",
        start: 0,
        end: 266,
        duration: 266,
        durationLabel: "4:26",
        cover: "https://img.youtube.com/vi/id5_3dKvEBg/mqdefault.jpg",
        sourceUrl: "https://www.youtube.com/watch?v=id5_3dKvEBg",
        description: "Festive devotional melody accompanied by traditional Dhak and Kanshor.",
        bengaliDesc: "শ্রেয়া ঘোষালের মিষ্টি সুরে কাঁসর-ঘণ্টা ও ঢাকের সাবেকি বন্দনা।"
      },
      {
        id: "durga-005",
        num: "05",
        title: "এবার যেন অন্যরকম পুজো (Ebar Jeno Onno Rokom Pujo)",
        title_bn: "এবার যেন অন্যরকম পুজো",
        title_en: "Ebar Jeno Onno Rokom Pujo",
        artist: "Nakash Aziz & Antara Mitra",
        composer: "Indraadip Dasgupta",
        lyricist: "Prasen",
        videoId: "E2zfQEo7Q_M",
        start: 0,
        end: 213,
        duration: 213,
        durationLabel: "3:33",
        cover: "https://img.youtube.com/vi/E2zfQEo7Q_M/mqdefault.jpg",
        sourceUrl: "https://www.youtube.com/watch?v=E2zfQEo7Q_M",
        description: "Modern romantic Puja anthem expressing festive nostalgia and friendship.",
        bengaliDesc: "শারদীয় প্রেমের মিষ্টি গান ও নতুন জামার রঙিন নস্টালজিয়া।"
      },
      {
        id: "durga-006",
        num: "06",
        title: "ঢাকের তালে কোমর দোলে (Dhaker Taale)",
        title_bn: "ঢাকের তালে কোমর দোলে",
        title_en: "Dhaker Taale Komor Dole",
        artist: "Abhijeet Bhattacharya (অভিজিৎ)",
        composer: "Jeet Gannguli",
        lyricist: "Priyo Chatterjee",
        videoId: "hbXuXt7gkFY",
        start: 0,
        end: 245,
        duration: 245,
        durationLabel: "4:05",
        cover: "https://img.youtube.com/vi/hbXuXt7gkFY/mqdefault.jpg",
        sourceUrl: "https://www.youtube.com/watch?v=hbXuXt7gkFY",
        description: "Timeless Bengali dance blockbuster played in every Puja pandal.",
        bengaliDesc: "মহাষ্টমীর ধুনুচি নাচে ও প্যান্ডেলে প্যান্ডেলে বাঙালির প্রাণের গান।"
      },
      {
        id: "durga-007",
        num: "07",
        title: "আমার দুগ্গা (Amar Dugga)",
        title_bn: "আমার দুগ্গা",
        title_en: "Amar Dugga",
        artist: "Anupam Roy (অনুপম রায়)",
        composer: "Anupam Roy",
        lyricist: "Anupam Roy",
        videoId: "4h5DXcN6cd4",
        start: 0,
        end: 282,
        duration: 282,
        durationLabel: "4:42",
        cover: "https://img.youtube.com/vi/4h5DXcN6cd4/mqdefault.jpg",
        sourceUrl: "https://www.youtube.com/watch?v=4h5DXcN6cd4",
        description: "Heartfelt modern Agomoni reflection on love, family reunions, and festive joy.",
        bengaliDesc: "অনুপম রায়ের কলমে ও সুরে শরৎকালীন অনুভূতির অপূর্ব মেলবন্ধন।"
      },
      {
        id: "durga-008",
        num: "08",
        title: "দুর্গে দুর্গে দুর্গতিনাশিনী (Durge Durge)",
        title_bn: "দুর্গে দুর্গে দুর্গতিনাশিনী",
        title_en: "Durge Durge Durgatinashini",
        artist: "Asha Bhosle (আশা ভোঁসলে)",
        composer: "R.D. Burman",
        lyricist: "Swapan Chakraborty",
        videoId: "45O8KBhSZ0I",
        start: 0,
        end: 320,
        duration: 320,
        durationLabel: "5:20",
        cover: "https://img.youtube.com/vi/45O8KBhSZ0I/mqdefault.jpg",
        sourceUrl: "https://www.youtube.com/watch?v=45O8KBhSZ0I",
        description: "A legendary classical devotional tribute to Maa Durga by Asha Bhosle.",
        bengaliDesc: "আশা ভোঁসলের অমর কণ্ঠে মহামায়ার মহিমান্বিত স্তুতি ও বন্দনা।"
      },
      {
        id: "durga-009",
        num: "09",
        title: "জাগো দুর্গা দশপ্রহরণধারিণী (Jago Durga)",
        title_bn: "জাগো দুর্গা দশপ্রহরণধারিণী",
        title_en: "Jago Durga Dashapraharanadharini",
        artist: "Dwijen Mukherjee & Chorus",
        composer: "Pankaj Kumar Mullick",
        lyricist: "Bani Kumar",
        videoId: "IfSJy3_Lkuo",
        start: 0,
        end: 290,
        duration: 290,
        durationLabel: "4:50",
        cover: "https://img.youtube.com/vi/IfSJy3_Lkuo/mqdefault.jpg",
        sourceUrl: "https://www.youtube.com/watch?v=IfSJy3_Lkuo",
        description: "The divine invocation awakening Maa Durga with orchestral grandeur.",
        bengaliDesc: "দ্বিজেন মুখোপাধ্যায়ের মেঘমন্দ্র কণ্ঠে মা দুর্গার শুভ জাগরণ।"
      },
      {
        id: "durga-010",
        num: "10",
        title: "বাজলো তোমার আলোর বেণু (Bajlo Tomar Alor Benu)",
        title_bn: "বাজলো তোমার আলোর বেণু",
        title_en: "Bajlo Tomar Alor Benu",
        artist: "Supriti Ghosh (সুপ্রীতি ঘোষ)",
        composer: "Pankaj Kumar Mullick",
        lyricist: "Bani Kumar",
        videoId: "IISWE2RiDBU",
        start: 0,
        end: 215,
        duration: 215,
        durationLabel: "3:35",
        cover: "https://img.youtube.com/vi/IISWE2RiDBU/mqdefault.jpg",
        sourceUrl: "https://www.youtube.com/watch?v=IISWE2RiDBU",
        description: "The dawn song that announces the arrival of autumn and Devi Durga.",
        bengaliDesc: "সুপ্রীতি ঘোষের কণ্ঠে শরতের স্নিগ্ধ প্রভাতে আলোর বেণু ধ্বনি।"
      },
      {
        id: "durga-011",
        num: "11",
        title: "অয়িগিরি নন্দিনী (পাওয়ার ফিউশন)",
        title_bn: "অয়িগিরি নন্দিনী (মহিষাসুরমর্দিনী স্তোত্র ফিউশন)",
        title_en: "Aigiri Nandini (Power Stotram Fusion)",
        artist: "Traditional Stotram / Studio Fusion Ensemble",
        composer: "Adi Shankaracharya (Adapted)",
        lyricist: "Sanskrit Mahakavya",
        videoId: "VY5U96vcJ3g",
        start: 0,
        end: 360,
        duration: 360,
        durationLabel: "6:00",
        cover: "https://img.youtube.com/vi/VY5U96vcJ3g/mqdefault.jpg",
        sourceUrl: "https://www.youtube.com/watch?v=VY5U96vcJ3g",
        description: "Electrifying high-octane rock-dhak fusion of the sacred Mahishasuramardini Stotram.",
        bengaliDesc: "আদি শঙ্করাচার্যের অয়িগিরি নন্দিনী স্তোত্রের তেজোদৃপ্ত আধুনিক ফিউশন।"
      },
      {
        id: "durga-012",
        num: "12",
        title: "আমাদের পুজোর গান (Amader Pujor Gaan)",
        title_bn: "আমাদের পুজোর গান",
        title_en: "Amader Pujor Gaan",
        artist: "Mimi Chakraborty (মিমি চক্রবর্তী)",
        composer: "Dabbu",
        lyricist: "Ritam Sen",
        videoId: "4c_EZMco1w8",
        start: 0,
        end: 210,
        duration: 210,
        durationLabel: "3:30",
        cover: "https://img.youtube.com/vi/4c_EZMco1w8/mqdefault.jpg",
        sourceUrl: "https://www.youtube.com/watch?v=4c_EZMco1w8",
        description: "Peppy modern Durga Puja celebration song with festive beats.",
        bengaliDesc: "মিমি চক্রবর্তীর প্রাণবন্ত কণ্ঠে নতুন প্রজন্মের শারদীয় উদ্‌যাপন।"
      }
    ]
  },

  ogMahalaya: {
    key: "ogMahalaya",
    label: "মহালয়া ও চণ্ডীপাঠ — মহিষাসুরমর্দিনী",
    pillLabel: "MAHALAYA BROADCAST",
    description: "বীরেন্দ্রকৃষ্ণ ভদ্রের উদাত্ত কণ্ঠের অবিনশ্বর মহিষাসুরমর্দিনী ও বাণীকুমারের রচনায় মূল আকাশবাণী সম্প্রচার।",
    sourceType: "youtube",
    tracksAreDistinctVideos: false,
    masterVideoId: "SFJeglBF5cg",
    tracks: [
      {
        id: "og-001",
        num: "০০",
        title: "সম্পূর্ণ মহালয়া মহিষাসুরমর্দিনী (আকাশবাণী মূল সম্প্রচার)",
        title_bn: "সম্পূর্ণ মহালয়া মহিষাসুরমর্দিনী (আকাশবাণী মূল সম্প্রচার)",
        title_en: "Complete Mahishasuramardini (AIR Original)",
        artist: "Birendra Krishna Bhadra & All India Radio Troupe",
        composer: "Pankaj Kumar Mullick",
        lyricist: "Bani Kumar",
        videoId: "SFJeglBF5cg",
        start: 0,
        end: 5240,
        duration: 5240,
        durationLabel: "87:20",
        cover: "https://img.youtube.com/vi/SFJeglBF5cg/mqdefault.jpg",
        description: "Complete uninterrupted broadcast of the iconic 1931 Mahishasura Mardini.",
        bengaliDesc: "বীরেন্দ্রকৃষ্ণ ভদ্রের চণ্ডীপাঠ ও পঙ্কজ মল্লিকের সুরারোপে সম্পূর্ণ আকাশবাণী মহালয়া।"
      },
      {
        id: "og-002",
        num: "০১",
        title: "১. ভূমিকা ও আগমনী শঙ্খধ্বনি",
        title_bn: "১. ভূমিকা ও আগমনী শঙ্খধ্বনি (দেবী আবাহন)",
        title_en: "1. Invocation & Sacred Shankha Call",
        artist: "AIR Orchestra & Conch Ensemble",
        composer: "Pankaj Kumar Mullick",
        lyricist: "Bani Kumar",
        videoId: "SFJeglBF5cg",
        start: 0,
        end: 420,
        duration: 420,
        durationLabel: "7:00",
        cover: "https://img.youtube.com/vi/SFJeglBF5cg/mqdefault.jpg",
        description: "The morning conch blow and poetic prologue heralding the autumn dawn.",
        bengaliDesc: "পবিত্র শঙ্খধ্বনি ও বাণীকুমারের ভূমিকা পাঠে দেবী বন্দনার সূচনা।"
      },
      {
        id: "og-003",
        num: "০২",
        title: "২. যা দেবী সর্বভূতেষু স্তোত্র ও আলোর বেণু",
        title_bn: "২. যা দেবী সর্বভূতেষু স্তোত্র ও আলোর বেণু",
        title_en: "2. Ya Devi Sarvabhuteshu & Alor Benu",
        artist: "Supriti Ghosh & Birendra Krishna Bhadra",
        composer: "Pankaj Kumar Mullick",
        lyricist: "Bani Kumar",
        videoId: "SFJeglBF5cg",
        start: 420,
        end: 1180,
        duration: 760,
        durationLabel: "12:40",
        cover: "https://img.youtube.com/vi/IISWE2RiDBU/mqdefault.jpg",
        description: "Devi Suktam stotrams invoking the Divine Mother in all living beings.",
        bengaliDesc: "যা দেবী সর্বভূতেষু মহাশক্তি বন্দনা ও আলোর বেণুর সুরে ভোরের আলো।"
      },
      {
        id: "og-004",
        num: "০৩",
        title: "৩. শ্রী শ্রী চণ্ডীপাঠ ও আশ্বিনের শারদপ্রাতে",
        title_bn: "৩. শ্রী শ্রী চণ্ডীপাঠ ও আশ্বিনের শারদপ্রাতে",
        title_en: "3. Sacred Chandipath & Ashwina Sharada",
        artist: "Birendra Krishna Bhadra & Dwijen Mukherjee",
        composer: "Pankaj Kumar Mullick",
        lyricist: "Bani Kumar",
        videoId: "SFJeglBF5cg",
        start: 1180,
        end: 2240,
        duration: 1060,
        durationLabel: "17:40",
        cover: "https://img.youtube.com/vi/IfSJy3_Lkuo/mqdefault.jpg",
        description: "The core Chandipath narration awakening Devi Durga to destroy darkness.",
        bengaliDesc: "বীরেন্দ্রকৃষ্ণ ভদ্রের চণ্ডীপাঠ ও 'জাগো দুর্গা' সমবেত সঙ্গীত।"
      },
      {
        id: "og-005",
        num: "০৪",
        title: "৪. রূপং দেহি জয়ং দেহি (দেবীস্তুতি ও মহামন্ত্র)",
        title_bn: "৪. রূপং দেহি জয়ং দেহি (দেবীস্তুতি ও মহামন্ত্র)",
        title_en: "4. Rupam Dehi Jayam Dehi Mahakavya",
        artist: "Birendra Krishna Bhadra & Mahalaya Choir",
        composer: "Pankaj Kumar Mullick",
        lyricist: "Markandeya Purana",
        videoId: "SFJeglBF5cg",
        start: 2240,
        end: 3450,
        duration: 1210,
        durationLabel: "20:10",
        cover: "https://img.youtube.com/vi/SFJeglBF5cg/mqdefault.jpg",
        description: "Devotional Argala Stotram chanting for blessings, victory, and prosperity.",
        bengaliDesc: "মহামন্ত্র রূপং দেহি জয়ং দেহি ও সমবেত ভক্তিমূলক অর্ঘ্য।"
      },
      {
        id: "og-006",
        num: "০৫",
        title: "৫. সিংহস্থা শশীশেখরা ও মহিষাসুর বধ",
        title_bn: "৫. সিংহস্থা শশীশেখরা ও মহিষাসুর বধ (ক্লাইম্যাক্স)",
        title_en: "5. Mahishasura Mardini Climax Battle",
        artist: "Birendra Krishna Bhadra, Shyamal Mitra, Tarun Banerjee",
        composer: "Pankaj Kumar Mullick",
        lyricist: "Bani Kumar",
        videoId: "SFJeglBF5cg",
        start: 3450,
        end: 4620,
        duration: 1170,
        durationLabel: "19:30",
        cover: "https://img.youtube.com/vi/45O8KBhSZ0I/mqdefault.jpg",
        description: "Dramatic retelling of Devi Durga's triumph over Mahishasura.",
        bengaliDesc: "মহিষাসুরমর্দিনীর ত্রিশূল নিক্ষেপ ও অসুর নিধনের জয়ধ্বনি।"
      },
      {
        id: "og-007",
        num: "০৬",
        title: "৬. শুভ আগমনী ও বিজয়া প্রণাম (শান্তি দিলে ভরি)",
        title_bn: "৬. শুভ আগমনী ও বিজয়া প্রণাম (শান্তি দিলে ভরি)",
        title_en: "6. Aashirbad & Concluding Stuti",
        artist: "Pankaj Kumar Mullick & All India Radio Chorus",
        composer: "Pankaj Kumar Mullick",
        lyricist: "Bani Kumar",
        videoId: "SFJeglBF5cg",
        start: 4620,
        end: 5240,
        duration: 620,
        durationLabel: "10:20",
        cover: "https://img.youtube.com/vi/SFJeglBF5cg/mqdefault.jpg",
        description: "Closing benediction showering peace and motherly blessings upon humanity.",
        bengaliDesc: "শান্তি দিলে ভরি সমাপ্তি গান ও শারদ মিলনের শুভ প্রণাম।"
      }
    ]
  },

  mahalayaSongs: {
    key: "mahalayaSongs",
    label: "চণ্ডীপাঠ ও মহালয়ার অমর গান",
    pillLabel: "MAHALAYA SONGS",
    description: "মহিষাসুরমর্দিনী আকাশবাণীর প্রতিটি গান ও স্তোত্রের পৃথক পৃথক অধ্যায়।",
    sourceType: "youtube",
    tracksAreDistinctVideos: false,
    masterVideoId: "SFJeglBF5cg",
    tracks: [
      {
        id: "ms-001",
        num: "01",
        title: "যা চণ্ডী মধুকৈটভাদিদৈত্যদলনী",
        title_bn: "যা চণ্ডী মধুকৈটভাদিদৈত্যদলনী",
        title_en: "Ya Chandi Madhukaitabhadi",
        artist: "Mahalaya Choir",
        composer: "Pankaj Kumar Mullick",
        videoId: "SFJeglBF5cg",
        start: 0,
        end: 220,
        duration: 220,
        durationLabel: "3:40",
        cover: "https://img.youtube.com/vi/SFJeglBF5cg/mqdefault.jpg"
      },
      {
        id: "ms-002",
        num: "02",
        title: "সিংহস্থা শশীশেখরা",
        title_bn: "সিংহস্থা শশীশেখরা",
        title_en: "Singhastha Shashishekhara",
        artist: "Birendra Krishna Bhadra",
        composer: "Pankaj Kumar Mullick",
        videoId: "SFJeglBF5cg",
        start: 220,
        end: 450,
        duration: 230,
        durationLabel: "3:50",
        cover: "https://img.youtube.com/vi/SFJeglBF5cg/mqdefault.jpg"
      },
      {
        id: "ms-003",
        num: "03",
        title: "বাজলো তোমার আলোর বেণু",
        title_bn: "বাজলো তোমার আলোর বেণু",
        title_en: "Bajlo Tomar Alor Benu",
        artist: "Supriti Ghosh & Birendra K. Bhadra",
        composer: "Pankaj Kumar Mullick",
        videoId: "IISWE2RiDBU",
        start: 0,
        end: 215,
        duration: 215,
        durationLabel: "3:35",
        cover: "https://img.youtube.com/vi/IISWE2RiDBU/mqdefault.jpg"
      },
      {
        id: "ms-004",
        num: "04",
        title: "জাগো দুর্গা দশপ্রহরণধারিণী",
        title_bn: "জাগো দুর্গা দশপ্রহরণধারিণী",
        title_en: "Jago Durga",
        artist: "Dwijen Mukherjee & Chorus",
        composer: "Pankaj Kumar Mullick",
        videoId: "IfSJy3_Lkuo",
        start: 0,
        end: 290,
        duration: 290,
        durationLabel: "4:50",
        cover: "https://img.youtube.com/vi/IfSJy3_Lkuo/mqdefault.jpg"
      },
      {
        id: "ms-005",
        num: "05",
        title: "ওগো আমার আগমনী-আলো",
        title_bn: "ওগো আমার আগমনী-আলো",
        title_en: "Ogo Amar Agomoni Alo",
        artist: "Shipra Basu",
        composer: "Pankaj Kumar Mullick",
        videoId: "SFJeglBF5cg",
        start: 1100,
        end: 1350,
        duration: 250,
        durationLabel: "4:10",
        cover: "https://img.youtube.com/vi/SFJeglBF5cg/mqdefault.jpg"
      },
      {
        id: "ms-006",
        num: "06",
        title: "তব অচিন্ত্য রূপ-চরিত-মহিমা",
        title_bn: "তব অচিন্ত্য রূপ-চরিত-মহিমা",
        title_en: "Tobo Ochintho Rupa Chorito Mohima",
        artist: "Mahalaya Chandipath",
        composer: "Pankaj Kumar Mullick",
        videoId: "SFJeglBF5cg",
        start: 1350,
        end: 1600,
        duration: 250,
        durationLabel: "4:10",
        cover: "https://img.youtube.com/vi/SFJeglBF5cg/mqdefault.jpg"
      },
      {
        id: "ms-007",
        num: "07",
        title: "অহং রুদ্রো বেদমন্ত্র",
        title_bn: "অহং রুদ্রো বেদমন্ত্র",
        title_en: "Aham Rudrebhir Vasubhishcharami",
        artist: "Vedic Chants Ensemble",
        composer: "Rigveda Chants",
        videoId: "SFJeglBF5cg",
        start: 1600,
        end: 1850,
        duration: 250,
        durationLabel: "4:10",
        cover: "https://img.youtube.com/vi/SFJeglBF5cg/mqdefault.jpg"
      },
      {
        id: "ms-008",
        num: "08",
        title: "অখিল-বিমানে তব জয়-গানে",
        title_bn: "অখিল-বিমানে তব জয়-গানে",
        title_en: "Akhilo Bimane Tobo Joyo Gaane",
        artist: "Krishna Dasgupta",
        composer: "Pankaj Kumar Mullick",
        videoId: "SFJeglBF5cg",
        start: 1850,
        end: 2100,
        duration: 250,
        durationLabel: "4:10",
        cover: "https://img.youtube.com/vi/SFJeglBF5cg/mqdefault.jpg"
      },
      {
        id: "ms-009",
        num: "09",
        title: "জয়ন্তী মঙ্গলা কালী স্তোত্র",
        title_bn: "জয়ন্তী মঙ্গলা কালী স্তোত্র",
        title_en: "Jayanti Mangala Kali Stotram",
        artist: "Devi Stuti Ensemble",
        composer: "Pankaj Kumar Mullick",
        videoId: "SFJeglBF5cg",
        start: 2100,
        end: 2350,
        duration: 250,
        durationLabel: "4:10",
        cover: "https://img.youtube.com/vi/SFJeglBF5cg/mqdefault.jpg"
      },
      {
        id: "ms-010",
        num: "10",
        title: "শুভ্র শঙ্খ-রবে",
        title_bn: "শুভ্র শঙ্খ-রবে",
        title_en: "Shubhro Shankha Robe",
        artist: "Mahalaya Chorus",
        composer: "Pankaj Kumar Mullick",
        videoId: "SFJeglBF5cg",
        start: 2350,
        end: 2600,
        duration: 250,
        durationLabel: "4:10",
        cover: "https://img.youtube.com/vi/SFJeglBF5cg/mqdefault.jpg"
      },
      {
        id: "ms-011",
        num: "11",
        title: "জটাজূট সমাযুক্তামার্ধেন্দুকৃতশেখরাম",
        title_bn: "জটাজূট সমাযুক্তামার্ধেন্দুকৃতশেখরাম",
        title_en: "Jatajuta Samayuktam",
        artist: "Birendra Krishna Bhadra",
        composer: "Pankaj Kumar Mullick",
        videoId: "SFJeglBF5cg",
        start: 2600,
        end: 2850,
        duration: 250,
        durationLabel: "4:10",
        cover: "https://img.youtube.com/vi/SFJeglBF5cg/mqdefault.jpg"
      },
      {
        id: "ms-012",
        num: "12",
        title: "নমো চণ্ডী, নমো চণ্ডী",
        title_bn: "নমো চণ্ডী, নমো চণ্ডী",
        title_en: "Namo Chandi, Namo Chandi",
        artist: "Aarti & Devotional Chants",
        composer: "Pankaj Kumar Mullick",
        videoId: "SFJeglBF5cg",
        start: 2850,
        end: 3100,
        duration: 250,
        durationLabel: "4:10",
        cover: "https://img.youtube.com/vi/SFJeglBF5cg/mqdefault.jpg"
      },
      {
        id: "ms-013",
        num: "13",
        title: "মা গো তব বীণে সঙ্গীত",
        title_bn: "মা গো তব বীণে সঙ্গীত",
        title_en: "Maa Go Tobo Beene",
        artist: "Shyamal Mitra",
        composer: "Pankaj Kumar Mullick",
        videoId: "SFJeglBF5cg",
        start: 3100,
        end: 3350,
        duration: 250,
        durationLabel: "4:10",
        cover: "https://img.youtube.com/vi/SFJeglBF5cg/mqdefault.jpg"
      },
      {
        id: "ms-014",
        num: "14",
        title: "বিমানে বিমানে",
        title_bn: "বিমানে বিমানে",
        title_en: "Bimane Bimane",
        artist: "Mahalaya Chorus",
        composer: "Pankaj Kumar Mullick",
        videoId: "SFJeglBF5cg",
        start: 3350,
        end: 3600,
        duration: 250,
        durationLabel: "4:10",
        cover: "https://img.youtube.com/vi/SFJeglBF5cg/mqdefault.jpg"
      },
      {
        id: "ms-015",
        num: "15",
        title: "জয় জয় জাপ্যজয়ে",
        title_bn: "জয় জয় জাপ্যজয়ে",
        title_en: "Jayo Jayo Japyajaye",
        artist: "All India Radio Chorus",
        composer: "Pankaj Kumar Mullick",
        videoId: "SFJeglBF5cg",
        start: 3600,
        end: 3850,
        duration: 250,
        durationLabel: "4:10",
        cover: "https://img.youtube.com/vi/SFJeglBF5cg/mqdefault.jpg"
      },
      {
        id: "ms-016",
        num: "16",
        title: "হে চিন্ময়ী",
        title_bn: "হে চিন্ময়ী",
        title_en: "Hey Chinmoyee",
        artist: "Tarun Banerjee",
        composer: "Pankaj Kumar Mullick",
        videoId: "SFJeglBF5cg",
        start: 3850,
        end: 4100,
        duration: 250,
        durationLabel: "4:10",
        cover: "https://img.youtube.com/vi/SFJeglBF5cg/mqdefault.jpg"
      },
      {
        id: "ms-017",
        num: "17",
        title: "অমল-কিরণে ত্রিভুবন-মনোহরিণী",
        title_bn: "অমল-কিরণে ত্রিভুবন-মনোহরিণী",
        title_en: "Amolo Kirone Tribhubana Monoharini",
        artist: "Pankaj Kumar Mullick",
        composer: "Pankaj Kumar Mullick",
        videoId: "SFJeglBF5cg",
        start: 4100,
        end: 4400,
        duration: 300,
        durationLabel: "5:00",
        cover: "https://img.youtube.com/vi/SFJeglBF5cg/mqdefault.jpg"
      },
      {
        id: "ms-018",
        num: "18",
        title: "জয়ন্তী মঙ্গলা কালী — পঙ্কজ মল্লিক",
        title_bn: "জয়ন্তী মঙ্গলা কালী — পঙ্কজ মল্লিক",
        title_en: "Jayanti Mangala Kali Solo",
        artist: "Pankaj Kumar Mullick",
        composer: "Pankaj Kumar Mullick",
        videoId: "SFJeglBF5cg",
        start: 4400,
        end: 4700,
        duration: 300,
        durationLabel: "5:00",
        cover: "https://img.youtube.com/vi/SFJeglBF5cg/mqdefault.jpg"
      },
      {
        id: "ms-019",
        num: "19",
        title: "শান্তি দিলে ভরি সমাপ্তি শ্লোক",
        title_bn: "শান্তি দিলে ভরি সমাপ্তি শ্লোক",
        title_en: "Shanti Dile Bhori Closing Hymn",
        artist: "Birendra Krishna Bhadra & Choir",
        composer: "Pankaj Kumar Mullick",
        videoId: "SFJeglBF5cg",
        start: 4700,
        end: 5240,
        duration: 540,
        durationLabel: "9:00",
        cover: "https://img.youtube.com/vi/SFJeglBF5cg/mqdefault.jpg"
      }
    ]
  },

  dhakVibes: {
    key: "dhakVibes",
    label: "ঢাকের বোল ও সাবেকি বাজনা",
    pillLabel: "DHAK BEATS",
    description: "মহাষ্টমী, নবমী ও সন্ধিপূজার খাঁটি কাঁসর-ঢাকের উন্মাদনা ও ঐতিহ্যবাহী উৎসবের তাল।",
    sourceType: "youtube",
    tracksAreDistinctVideos: false,
    masterVideoId: "8EA8JrDMZbM",
    tracks: [
      {
        id: "dhak-001",
        num: "০১",
        title: "১. রথের বাজনা (রথযাত্রা শোভাযাত্রা)",
        title_bn: "১. রথের বাজনা (রথযাত্রা শোভাযাত্রা)",
        title_en: "1. Rathyatra Grand Procession",
        artist: "Master Dhakis of Bengal",
        videoId: "8EA8JrDMZbM",
        start: 0,
        end: 180,
        duration: 180,
        durationLabel: "3:00",
        cover: "https://img.youtube.com/vi/8EA8JrDMZbM/mqdefault.jpg",
        description: "Lively celebratory processional groove played during Rathyatra and early festival marches.",
        bengaliDesc: "রথযাত্রা ও উৎসব শোভাযাত্রার প্রাণবন্ত ও আনন্দঘন প্রারম্ভিক তাল।"
      },
      {
        id: "dhak-002",
        num: "০২",
        title: "২. আগমনী ও বরণ বাজনা (মা আসছেন ঘরে)",
        title_bn: "২. আগমনী ও বরণ বাজনা (মা আসছেন ঘরে)",
        title_en: "2. Agomoni Boron Rhythms",
        artist: "Master Dhakis of Bengal",
        videoId: "8EA8JrDMZbM",
        start: 180,
        end: 360,
        duration: 180,
        durationLabel: "3:00",
        cover: "https://img.youtube.com/vi/8EA8JrDMZbM/mqdefault.jpg",
        description: "Emotional welcoming beats played when Maa Durga enters the Natmandir.",
        bengaliDesc: "বোধন ও ঠাকুর বরণের সময় খাঁটি কাঁসর ও ঢাকের আবেগময় আগমনী বোল।"
      },
      {
        id: "dhak-003",
        num: "০৩",
        title: "৩. সন্ধ্যা আরতি ও ধুনুচি নৃত্য (নাটমন্দির আরতি)",
        title_bn: "৩. সন্ধ্যা আরতি ও ধুনুচি নৃত্য (নাটমন্দির আরতি)",
        title_en: "3. Sandhya Aarti & Dhunuchi Dance",
        artist: "Master Dhakis of Bengal",
        videoId: "8EA8JrDMZbM",
        start: 360,
        end: 600,
        duration: 240,
        durationLabel: "4:00",
        cover: "https://img.youtube.com/vi/8EA8JrDMZbM/mqdefault.jpg",
        description: "Fast, electrifying tempo matching smoky coconut-husk dhunuchi dances.",
        bengaliDesc: "ধুনুচি নাচের সাথে কাঁসর-ঘণ্টার দ্রুত ও উন্মাদনাময় তাল।"
      },
      {
        id: "dhak-004",
        num: "০৪",
        title: "৪. সন্ধিপূজা ও ১০৮ প্রদীপ মহোৎসব (চামুণ্ডা বোধন)",
        title_bn: "৪. সন্ধিপূজা ও ১০৮ প্রদীপ মহোৎসব (চামুণ্ডা বোধন)",
        title_en: "4. Sandhi Puja 108 Deepam Call",
        artist: "Master Dhakis of Bengal",
        videoId: "8EA8JrDMZbM",
        start: 600,
        end: 840,
        duration: 240,
        durationLabel: "4:00",
        cover: "https://img.youtube.com/vi/8EA8JrDMZbM/mqdefault.jpg",
        description: "Intense climactic rhythmic surge during Ashtami-Navami junction.",
        bengaliDesc: "অষ্টমী ও নবমীর সন্ধিক্ষণে চামুণ্ডা রূপের আবাহনে ১০৮ প্রদীপের মহাতাল।"
      },
      {
        id: "dhak-005",
        num: "০৫",
        title: "৫. বিসর্জন ও বিজয়া শোভাযাত্রা (ভাসান শোভাযাত্রা)",
        title_bn: "৫. বিসর্জন ও বিজয়া শোভাযাত্রা (ভাসান শোভাযাত্রা)",
        title_en: "5. Bisarjan Procession & Sindoor Khela",
        artist: "Master Dhakis of Bengal",
        videoId: "8EA8JrDMZbM",
        start: 840,
        end: 1100,
        duration: 260,
        durationLabel: "4:20",
        cover: "https://img.youtube.com/vi/8EA8JrDMZbM/mqdefault.jpg",
        description: "Bittersweet victory beats echoing through riverbank farewell marches.",
        bengaliDesc: "দশমীর সিঁদুর খেলা ও গঙ্গাবক্ষে বিসর্জনের বিদায়ী সুর।"
      },
      {
        id: "dhak-006",
        num: "০৬",
        title: "৬. এক কাঠি ও দুক কাঠি যুগলবন্দী (মাস্টার একক বোল)",
        title_bn: "৬. এক কাঠি ও দুক কাঠি যুগলবন্দী (মাস্টার একক বোল)",
        title_en: "6. Master Solo Sticks Duet",
        artist: "Master Dhakis of Bengal",
        videoId: "8EA8JrDMZbM",
        start: 1100,
        end: 1320,
        duration: 220,
        durationLabel: "3:40",
        cover: "https://img.youtube.com/vi/8EA8JrDMZbM/mqdefault.jpg",
        description: "Virtuosic exhibition of intricate stick rudiments and syncopations.",
        bengaliDesc: "ঢাকের জটিল কাঠির বোল ও নিপুণ সাবেকি বোলবাণীর যুগলবন্দী।"
      }
    ]
  }
};

export const nativePujoData = {
  pujoName: "মণ্ডল বাড়ির পুজো",
  bengaliName: "মণ্ডল বাড়ির পুজো (ফুরফুরা মণ্ডল পরিবার)",
  englishName: "Furfura Mondol Poribar Pujo",
  tagline: "Celebrating 30th Year of Heritage & Devotion (Est. 1997)",
  bengaliTagline: "১৯৯৭ সাল থেকে প্রতিষ্ঠিত ঐতিহ্য, সাবেকি রীতিনীতি ও আন্তরিক মিলনোৎসব",
  instagramHandle: "@furfura_mondal_poribar",
  instagramUrl: "https://www.instagram.com/furfura_mondal_poribar?igsh=d3BvbzY5NTI5Z2hp",
  location: "Furfura Mondol Poribar Natmandir, Furfura, Kazipada, Hooghly, Bengal",
  history: {
    title: "মণ্ডল বাড়ির পুজোর ঐতিহ্য ও ইতিহাস",
    subtitle: "30th Year of Heritage & Ancestral Devotion • Est. 1997",
    desc: "মণ্ডল বাড়ির পুজো শুধুমাত্র একটি উৎসব নয়, এটি আমাদের পারিবারিক ভালোবাসা, শিকড় ও সনাতন সংস্কৃতির অমলিন মেলবন্ধন। ১৯৯৭ সাল থেকে নিরবচ্ছিন্নভাবে উদযাপিত হয়ে আসা এই পুজোয় আজও সংরক্ষিত রয়েছে নিষ্ঠাপূর্ণ সাবেকি প্রথা। সাবেকি একচালা ডাকের সাজের প্রতিমা, কূলপুরোহিতের কণ্ঠে বিশুদ্ধ চণ্ডীপাঠ, ১০৮ পদ্ম ও মাটির প্রদীপে সন্ধিপূজা, আর নাটমন্দির কাঁপানো ঐতিহ্যবাহী ঢাকের বোলে প্রতিবছর ঘরে ফিরে আসেন ঘরের মেয়ে মা দুর্গা।",
    features: [
      "ঐতিহ্যবাহী সাবেকি একচালা ডাকের সাজের প্রতিমা",
      "সন্ধিপূজায় ১০৮ পদ্ম ও ১০৮ মাটির প্রদীপ নিবেদন",
      "মহাষ্টমীর পুণ্যলগ্নে কুমারী পূজা ও হোমযজ্ঞ",
      "মহা নবমীর সন্ধ্যায় বিশেষ ধুনুচি নাচ ও ঢাকের যুগলবন্দী",
      "দশমীর সিঁদুর খেলা ও প্রীতি বিজয়ার মিষ্টিমুখ"
    ]
  },
  dates2026: [
    {
      id: "mahalaya",
      bengaliDay: "মহালয়া",
      englishDay: "Mahalaya",
      date: "১০ অক্টোবর ২০২৬ (শনিবার)",
      englishDate: "10 October 2026 (Saturday)",
      rituals: "ভোর ৪:৩০ মহালয়া চণ্ডীপাঠ, তর্পণ ও দেবী আবাহন",
      englishRituals: "4:30 AM Dawn Chandi Path, Tarpan & Devi Invocation"
    },
    {
      id: "sasthi",
      bengaliDay: "মহা ষষ্ঠী",
      englishDay: "Maha Shasthi",
      date: "১৬ অক্টোবর ২০২৬ (শুক্রবার)",
      englishDate: "16 October 2026 (Friday)",
      rituals: "দেবী বোধন, আমন্ত্রণ, অধিবাস ও সান্ধ্য আরতি",
      englishRituals: "Devi Bodhon, Amantran, Adhibas & Evening Aarti"
    },
    {
      id: "saptami",
      bengaliDay: "মহা সপ্তমী",
      englishDay: "Maha Saptami",
      date: "১৭ অক্টোবর ২০২৬ (শনিবার)",
      englishDate: "17 October 2026 (Saturday)",
      rituals: "নবপত্রিকা (কলাবউ) স্নান ও প্রবেশ, সপ্তমী বিহিত পূজা ও অঞ্জলি",
      englishRituals: "Nabapatrika (Kola Bou) Bathing & Natmandir Entry, Saptami Puja & Anjali"
    },
    {
      id: "astami",
      bengaliDay: "মহা অষ্টমী ও সন্ধিপূজা",
      englishDay: "Maha Ashtami & Sandhi Puja",
      date: "১৮ অক্টোবর ২০২৬ (রবিবার)",
      englishDate: "18 October 2026 (Sunday)",
      rituals: "মহাষ্টমী পুষ্পাঞ্জলি, কুমারী পূজা ও ১০৮ পদ্ম-প্রদীপে সন্ধিপূজা",
      englishRituals: "Maha Ashtami Pushpanjali, Kumari Puja & 108 Lotuses Sandhi Puja"
    },
    {
      id: "navami",
      bengaliDay: "মহা নবমী ও ধুনুচি নাচ",
      englishDay: "Maha Nabami & Dhunuchi Dance",
      date: "১৯ অক্টোবর ২০২৬ (সোমবার)",
      englishDate: "19 October 2026 (Monday)",
      rituals: "নবমী হোমযজ্ঞ, মহাপ্রসাদ অন্নভোগ বিতরণ ও সান্ধ্য ধুনুচি নৃত্য",
      englishRituals: "Nabami Homa Fire Ritual, Mahaprasad Feast Distribution & Evening Dhunuchi Dance"
    },
    {
      id: "dashami",
      bengaliDay: "বিজয়া দশমী ও সিঁদুর খেলা",
      englishDay: "Bijoya Dashami & Sindoor Khela",
      date: "২০ অক্টোবর ২০২৬ (মঙ্গলবার)",
      englishDate: "20 October 2026 (Tuesday)",
      rituals: "অপরাজিতা পূজা, দেবী বরণ, সিঁদুর খেলা, মিষ্টিমুখ ও শান্তিজল",
      englishRituals: "Aparajita Puja, Devi Baran, Sindoor Khela & Shantijal Blessings"
    }
  ],
  highlights: [
    {
      title: "১৯৯৭ থেকে প্রতিষ্ঠিত ঐতিহ্য",
      englishTitle: "Ancestral Legacy since 1997",
      desc: "সাবেকি একচালা শোলার ডাকের সাজের প্রতিমা ও নিষ্ঠার সাথে রক্ষিত বৈদিক পূজা আচার।",
      englishDesc: "Traditional Ekchala Sholar Daker Saaj idol and sacred Vedic rituals preserved with utmost reverence."
    },
    {
      title: "সন্ধিপূজা ও ১০৮ পদ্ম নিবেদন",
      englishTitle: "108 Lotuses Sandhi Puja",
      desc: "অষ্টমী-নবমীর সন্ধিক্ষণে ১০৮ মাটির প্রদীপ ও নীলপদ্মে দেবী চামুণ্ডার মহাশক্তি আরাধনা।",
      englishDesc: "Grand worship of Devi Chamunda at the cusp of Ashtami-Navami with 108 earthen lamps and sacred blue lotuses."
    },
    {
      title: "মহাপ্রসাদ ও অন্নভোগ বিতরণ",
      englishTitle: "Universal Mahaprasad Feast",
      desc: "ঘিয়ে ভাজা লুচি, ভোগের খিচুড়ি, লাবড়া, চাটনি, পায়েশ ও নারকেল নাড়ুর সার্বজনীন বিতরণ।",
      englishDesc: "Ghee-fried Luchi, Bhog Khichuri, Labra, Chutney, Payesh and handmade Narkel Naru for all devotees."
    },
    {
      title: "ধুনুচি নাচ ও ঢাকের যুগলবন্দী",
      englishTitle: "Dhunuchi Dance & Master Dhak",
      desc: "নবমীর সন্ধ্যায় কাঁসর-ঘণ্টার তালে ধুনুচি আরতি ও সাবেকি ঢাকের তাকডুম তাকডুম লড়াই।",
      englishDesc: "High-energy evening Aarti with fragrant coconut husk incense, brass gongs, and twin-stick Dhak rhythm duels."
    }
  ],
  galleryCategories: [
    { id: "all", label: "সমস্ত ছবি", icon: "" },
    { id: "protima", label: "প্রতিমা দর্শন", icon: "" },
    { id: "aarti", label: "ধুনুচি ও আরতি", icon: "" },
    { id: "heritage", label: "ঐতিহ্য ও পরিবার", icon: "" },
    { id: "sharat", label: "শরতের আগমনী", icon: "" },
    { id: "community", label: "ভক্তদের আপলোড", icon: "" }
  ],
  gallery: [
    {
      id: "gal-1",
      category: "protima",
      categoryLabel: "প্রতিমা ও বরণ",
      title: "Mondol Bari Traditional Ekchala Protima",
      bengaliTitle: "সাবেকি একচালা ডাকের সাজে মা দুর্গা",
      desc: "Sacred divine Darshan with traditional clay craftsmanship, Sholar Daak ornaments, and ancestral devotion since 1997.",
      bengaliDesc: "১৯৯৭ সাল থেকে নিষ্ঠার সাথে পূজিত সাবেকি একচালা ডাকের সাজের প্রতিমার দিব্য দর্শন।",
      author: "মন্ডল পরিবার মহাফেজখানা",
      likes: 356,
      src: "/gallery/art_hero_idol.webp"
    },
    {
      id: "gal-2",
      category: "aarti",
      categoryLabel: "ধুনুচি ও আরতি",
      title: "Sandhya Aarti & 108 Deepam Illumination",
      bengaliTitle: "সন্ধ্যা আরতি ও ১০৮ প্রদীপ প্রজ্বলন",
      desc: "Evening aarti lighting illuminating the historic Natmandir courtyard with golden diya glow and sacred incense smoke.",
      bengaliDesc: "নাটমন্দিরে ধুনুচির ধোঁয়া, মাটির প্রদীপের আলো ও কাঁসর-ঘণ্টার বোলে সন্ধ্যার মহাশক্তি আরতি।",
      author: "ফুরফুরা মণ্ডল পরিবার",
      likes: 294,
      src: "/gallery/user_dhunuchi_01.webp"
    },
    {
      id: "gal-3",
      category: "heritage",
      categoryLabel: "ঐতিহ্য ও পরিবার",
      title: "Mondol Bari Festive Night Grandeur",
      bengaliTitle: "মন্ডল বাড়ির উৎসবের আলোকসজ্জা ও মিলনমেলা",
      desc: "Night celebrations uniting family, relatives, neighbors and devotees under illuminated Bonedi heritage arches.",
      bengaliDesc: "উৎসবের আলোয় ঝলমলে বনেদি নাটমন্দির প্রাঙ্গণ ও বহু দূর-দূরান্ত থেকে আসা ভক্তদের আন্তরিক মিলনমেলা।",
      author: "মন্ডল পরিবার সংগ্রহ",
      likes: 218,
      src: "/gallery/user_dhunuchi_05.webp"
    },
    {
      id: "gal-4",
      category: "sharat",
      categoryLabel: "শরতের আগমনী",
      title: "Early Morning Mahalaya & Chandi Path",
      bengaliTitle: "ভোরের আগমনী ও চণ্ডীপাঠের সুর",
      desc: "Reverberating sound of sacred Shankha, dawn Chandi Path, and morning mist welcoming Devi Uma.",
      bengaliDesc: "ভোরের শিশিরভেজা শিউলিতলায় বীরেন্দ্রকৃষ্ণ ভদ্রের চণ্ডীপাঠ ও শঙ্খধ্বনিতে শারদ আবাহন।",
      author: "ঐতিহ্য স্মৃতি ভাণ্ডার",
      likes: 275,
      src: "/gallery/art_dawn_mahalaya.webp"
    },
    {
      id: "gal-5",
      category: "crafts",
      categoryLabel: "হস্তশিল্প ও আলপনা",
      title: "Traditional Handcrafted Rice Flour Alpana",
      bengaliTitle: "নাটমন্দির চত্বরে সাবেকি চালের গুঁড়োর আলপনা",
      desc: "Authentic handmade sacred floor art intricately drawn with rice flour paste across the Mondol Bari Natmandir.",
      bengaliDesc: "শালুক, পদ্ম ও দেবী চরণের নিখুঁত হাতের টানে আঁকা মন্ডল বাড়ির ঐতিহ্যবাহী সাবেকি পিটুলি আলপনা।",
      author: "ফুরফুরা মণ্ডল পরিবার",
      likes: 389,
      src: "/gallery/user_alpona.webp"
    },
    {
      id: "gal-6",
      category: "aarti",
      categoryLabel: "ধুনুচি ও আরতি",
      title: "Dhunuchi Dance in Ancestral Courtyard",
      bengaliTitle: "নাটমন্দিরে ধুনুচি নৃত্যের উন্মাদনা",
      desc: "Sacred incense ecstasy with twin earthen dhunuchis, rhythmic dhak beats, and traditional brass gongs.",
      bengaliDesc: "ধুনোর গন্ধ ও কাঁসর-ঘণ্টার বোলে মন্ডল বাড়ির প্রাঙ্গণে আন্তরিক ধুনুচি নৃত্যের চিরন্তন স্মৃতি।",
      author: "মন্ডল পরিবার মহাফেজখানা",
      likes: 342,
      src: "/gallery/user_dhunuchi_03.webp"
    }
  ]
};

export const liveDhakMeta = {
  videoId: "8EA8JrDMZbM",
  title: "TYPES OF DHAK PLAYING",
  englishTitle: "Types of Traditional Dhak Playing (Authentic Master Performance)",
  bengaliTitle: "ঐতিহ্যবাহী সাবেকি ঢাকের বিভিন্ন বোল ও বাদন শৈলী",
  totalDuration: 109,
  totalParts: 6,
  cover: "https://img.youtube.com/vi/8EA8JrDMZbM/mqdefault.jpg",
  sourceUrl: "https://www.youtube.com/watch?v=8EA8JrDMZbM"
};

export const authenticLiveDhakParts = [
  {
    id: "live-dhak-1",
    partNumber: 1,
    num: "01",
    bengaliNum: "১",
    title_bn: "রথের বাজনা (রথযাত্রা শোভাযাত্রা)",
    title_en: "1. Rot / Ratha Yatra Procession",
    subtitle_bn: "ভারী ডুগি বেস ও দোদুল দোল সহ শোভাযাত্রার প্রাণবন্ত তাল",
    start: 0,
    end: 18,
    duration: 18,
    durationLabel: "0:18",
    tempoBpm: 112,
    taal: "Processional Driving Kaharwa",
    style: "procession_swing",
    styleLabel: "শোভাযাত্রার বোল",
    videoId: "8EA8JrDMZbM",
    cover: "https://img.youtube.com/vi/8EA8JrDMZbM/mqdefault.jpg",
    description: "Processional driving swing with heavy bass pulses. Stately forward momentum traditionally played during sacred Ratha Yatra and temple deity processions.",
    bengaliDesc: "রথযাত্রা ও দেবীর শোভাযাত্রায় বাজানো গুরুগম্ভীর দোলায়িত বোল। এক কাঠির ভারী বেস ও কঞ্চির ছন্দবদ্ধ কাঁপনে চতুর্দিক মুখরিত হয়ে ওঠে।",
    acousticDetails: "80Hz deep barrel hide resonance, locked swing groove on beats 1 & 3, driving Kanchi snaps."
  },
  {
    id: "live-dhak-2",
    partNumber: 2,
    num: "02",
    bengaliNum: "২",
    title_bn: "আগমনী ও বরণ বাজনা (মা আসছেন ঘরে)",
    title_en: "2. Agomoni & Boron Invocation",
    subtitle_bn: "এক কাঠির গম্ভীর আহ্বান ও দেবী বরণের সাবেকি লয়",
    start: 18,
    end: 36,
    duration: 18,
    durationLabel: "0:18",
    tempoBpm: 92,
    taal: "Traditional Agomoni 8-Matra Devotional",
    style: "ak_kathi",
    styleLabel: "এক কাঠি বরণ",
    videoId: "8EA8JrDMZbM",
    cover: "https://img.youtube.com/vi/8EA8JrDMZbM/mqdefault.jpg",
    description: "Reverent Ak-kathi welcoming beat for Mother Durga's arrival at the Natmandir. Resonant single-hand bass resonance with disciplined bamboo stick punctuation.",
    bengaliDesc: "দেবী বোধন ও নবপত্রিকা বরণের পবিত্র আগমনী লয়। এক হাতের গম্ভীর বেস ও অন্য হাতে কঞ্চির অবিরাম নিয়ন্ত্রিত বোলে দেবীর আবাহন সূচিত হয়।",
    acousticDetails: "Deep open Dha pulse (74Hz), wrist damping control, solemn welcoming cadence."
  },
  {
    id: "live-dhak-3",
    partNumber: 3,
    num: "03",
    bengaliNum: "৩",
    title_bn: "সন্ধ্যা আরতি ও ধুনুচি নৃত্য (নাটমন্দির আরতি)",
    title_en: "3. Sandhya Aarti & Dhunuchi Dance",
    subtitle_bn: "ধুনোর গন্ধ ও কাঁসর ঘণ্টার উন্মাদনায় দ্রুতলয় যুগলবন্দী",
    start: 36,
    end: 54,
    duration: 18,
    durationLabel: "0:18",
    tempoBpm: 124,
    taal: "Dhunuchi Kaharwa Drut Polyrhythm",
    style: "duk_kathi",
    styleLabel: "দুক কাঠি আরতি",
    videoId: "8EA8JrDMZbM",
    cover: "https://img.youtube.com/vi/8EA8JrDMZbM/mqdefault.jpg",
    description: "High-energy syncopated polyrhythms with Dhuno smoke and Kanshor. Twin bamboo sticks create blistering rolls locking with evening incense dancers.",
    bengaliDesc: "সন্ধ্যা আরতির সময় ধুনুচি নৃত্যের উন্মাদনাময় দ্রুত বোল। জোড়া কঞ্চির ক্ষিপ্র আঘাতে কাঁসর ঘণ্টার সাথে সৃষ্টি হয় সম্মোহনী ছন্দ।",
    acousticDetails: "High tension 2.1kHz Kanchi crackles, interlocking syncopated bell tones, electrifying groove."
  },
  {
    id: "live-dhak-4",
    partNumber: 4,
    num: "04",
    bengaliNum: "৪",
    title_bn: "সন্ধিপূজা ও ১০৮ প্রদীপ মহোৎসব (চামুণ্ডা বোধন)",
    title_en: "4. Sandhi Puja 108 Deepam Climax",
    subtitle_bn: "অষ্টমী-নবমীর সন্ধিক্ষণে বজ্রনির্ঘোষ দ্রুত ঘূর্ণন ও রোল",
    start: 54,
    end: 72,
    duration: 18,
    durationLabel: "0:18",
    tempoBpm: 128,
    tempoClimaxBpm: 144,
    taal: "Sandhi Chhanda Accelerando Roll",
    style: "accelerando",
    styleLabel: "মহোৎসব চরম লয়",
    videoId: "8EA8JrDMZbM",
    cover: "https://img.youtube.com/vi/8EA8JrDMZbM/mqdefault.jpg",
    description: "Thunderous rolling accelerando and dramatic drum rolls played as 108 clay lamps and blue lotuses are offered to Devi Chamunda.",
    bengaliDesc: "সন্ধিপূজার মাহেন্দ্রক্ষণে ১০৮ পদ্ম ও প্রদীপ নিবেদনের চরম মুহূর্ত। বিদ্যুৎগতির রোল ও ক্রমাগত লয় বৃদ্ধি এক স্বর্গীয় পরিবেশ সৃষ্টি করে।",
    acousticDetails: "Rapid double-stick tremolo, continuous accelerando crescendo, explosive dynamic range."
  },
  {
    id: "live-dhak-5",
    partNumber: 5,
    num: "05",
    bengaliNum: "৫",
    title_bn: "বিসর্জন ও বিজয়া শোভাযাত্রা (ভাসান শোভাযাত্রা)",
    title_en: "5. Bishorjan / Bisarjan Procession",
    subtitle_bn: "আসছে বছর আবার হবে — আনন্দ ও বিদায়ের আবেগী ছন্দ",
    start: 72,
    end: 92,
    duration: 20,
    durationLabel: "0:20",
    tempoBpm: 120,
    taal: "Bisarjan Shobhajatra Emotional Swing",
    style: "bisarjan_swing",
    styleLabel: "বিসর্জন দোল",
    videoId: "8EA8JrDMZbM",
    cover: "https://img.youtube.com/vi/8EA8JrDMZbM/mqdefault.jpg",
    description: "Fast, joyous yet emotional farewell groove for Maa Durga's immersion procession to the holy Ganges. Deep syncopation with infectious festival bounce.",
    bengaliDesc: "দেবী বিসর্জনের আবেগঘন কিন্তু প্রাণবন্ত বিদায় বোল। ভারী খটকা ও দোদুল নাচের তালে ভক্তদের কণ্ঠে ধ্বনিত হয় 'আসছে বছর আবার হবে'।",
    acousticDetails: "Heavy syncopated bass on beats 1 & 4, shimmering high-register stick dance, emotional swing."
  },
  {
    id: "live-dhak-6",
    partNumber: 6,
    num: "06",
    bengaliNum: "৬",
    title_bn: "এক কাঠি ও দুক কাঠি যুগলবন্দী (মাস্টার একক বোল)",
    title_en: "6. Ak-Kathi & Duk-Kathi Master Solo",
    subtitle_bn: "ঢাকের যাদুকরী হাতের কাজ, মিউটেড খটকা ও ক্ষিপ্র কঞ্চি",
    start: 92,
    end: 109,
    duration: 17,
    durationLabel: "0:17",
    tempoBpm: 138,
    taal: "Virtuoso Solo Jugalbandi Teental",
    style: "jugalbandi_solo",
    styleLabel: "যুগলবন্দী একক",
    videoId: "8EA8JrDMZbM",
    cover: "https://img.youtube.com/vi/8EA8JrDMZbM/mqdefault.jpg",
    description: "Virtuosic solo showing wrist damping, ghost notes, and lightning-fast bamboo stick snaps. A masterclass in authentic Bengali Dhak mastery.",
    bengaliDesc: "ঢাকের খাঁটি শিল্পকলার প্রদর্শনী। কব্জির নিপুণ নিয়ন্ত্রণ, মিউটেড খটকা ও দ্রুতগতির কঞ্চি চাট্টির এক অনুপম যুগলবন্দী একক পরিবেশনা।",
    acousticDetails: "Micro-articulated finger mutes, lightning 16th-note stick snaps, full acoustic range showcase."
  }
];

export const TRADITIONAL_BOLS = [
  {
    id: 1,
    partNumber: 1,
    num: "01",
    bengaliNum: "১",
    title_bn: "১. রথের বাজনা (\"ড্যাং তাং ড্যাং তাং | ধা ধা গুড়গুড় ধা\")",
    title_en: "1. Rot / Ratha Yatra Procession Bol",
    tempoBpm: 112,
    style: "procession_swing",
    styleLabel: "শোভাযাত্রার বোল", styleLabelEnglish: "Procession Swing",
    taal: "Processional Driving Kaharwa (8-Matra Swing)",
    timeSignature: "4/4",
    swingFactor: 0.14,
    description: "Processional driving swing with heavy bass pulses. Stately forward momentum traditionally played during sacred Ratha Yatra and temple deity processions.",
    vocalPhoneticBol: "ড্যাং তাং ড্যাং তাং | ধা ধা গুড়গুড় ধা ||",
    romanizedBol: "Dyang Tang Dyang Tang | Dha Dha Gurgur Dha ||",
    barLengthSteps: 16,
    shankhaTriggers: [
      { step: 0, durationMs: 2000, description: "Grand opening conch blast" },
      { step: 8, durationMs: 1400, description: "Mid-phrase conch resonance" }
    ],
    stepSequence: [
      { step: 0, stroke: "DHA", stick: "LEFT_HAND", velocity: 1.00, accent: true, kansor: "CLANG_HIGH", phonetic: "ড্যাং (Dyang)" },
      { step: 1, stroke: "NONE", stick: "NONE", velocity: 0.0, accent: false, kansor: "NONE", phonetic: "-" },
      { step: 2, stroke: "DYANG", stick: "RIGHT_KANCHI", velocity: 0.90, accent: false, kansor: "RING_LOW", phonetic: "তাং (Tang)" },
      { step: 3, stroke: "KUT", stick: "LEFT_HAND", velocity: 0.50, accent: false, kansor: "NONE", phonetic: "কা (Ka)" },
      { step: 4, stroke: "DHA", stick: "LEFT_HAND", velocity: 0.95, accent: true, kansor: "CLANG_HIGH", phonetic: "ড্যাং (Dyang)" },
      { step: 5, stroke: "NONE", stick: "NONE", velocity: 0.0, accent: false, kansor: "NONE", phonetic: "-" },
      { step: 6, stroke: "DYANG", stick: "RIGHT_KANCHI", velocity: 0.90, accent: false, kansor: "RING_LOW", phonetic: "তাং (Tang)" },
      { step: 7, stroke: "NONE", stick: "NONE", velocity: 0.0, accent: false, kansor: "NONE", phonetic: "-" },
      { step: 8, stroke: "DHA", stick: "LEFT_HAND", velocity: 1.05, accent: true, kansor: "CLANG_HIGH", phonetic: "ধা (Dha)" },
      { step: 9, stroke: "NONE", stick: "NONE", velocity: 0.0, accent: false, kansor: "NONE", phonetic: "-" },
      { step: 10, stroke: "DHA", stick: "LEFT_HAND", velocity: 0.95, accent: false, kansor: "RING_LOW", phonetic: "ধা (Dha)" },
      { step: 11, stroke: "NONE", stick: "NONE", velocity: 0.0, accent: false, kansor: "NONE", phonetic: "-" },
      { step: 12, stroke: "GUR_GUR", stick: "BOTH_KANCHI", velocity: 0.88, accent: false, kansor: "CLANG_HIGH", phonetic: "গুড় (Gur)" },
      { step: 13, stroke: "GUR_GUR", stick: "BOTH_KANCHI", velocity: 0.92, accent: false, kansor: "NONE", phonetic: "গুড় (Gur)" },
      { step: 14, stroke: "DHA", stick: "LEFT_HAND", velocity: 1.05, accent: true, kansor: "RING_LOW", phonetic: "ধা (Dha)" },
      { step: 15, stroke: "NONE", stick: "NONE", velocity: 0.0, accent: false, kansor: "NONE", phonetic: "-" }
    ]
  },
  {
    id: 2,
    partNumber: 2,
    num: "02",
    bengaliNum: "২",
    title_bn: "২. আগমনী ও বরণ বাজনা (\"ড্যাং ড্যাং ড্যাংকা ড্যাং, মা আসছেন ঘরে\")",
    title_en: "2. Agomoni & Boron Invocation Bol",
    tempoBpm: 92,
    style: "ak_kathi",
    styleLabel: "এক কাঠি বরণ", styleLabelEnglish: "Ak-Kathi",
    taal: "Traditional Agomoni 8-Matra Devotional",
    timeSignature: "4/4",
    swingFactor: 0.08,
    description: "Welcoming invocation played as the Goddess enters the Thakur Dalan and during Nabapatrika Boron. Stately, reverent, with resonant Dha foundation and crisp Kanchi snap.",
    vocalPhoneticBol: "ড্যাং ড্যাং ড্যাংকা ড্যাং | তাগ ধা গুড় গুড় ধা ||",
    romanizedBol: "Dyang Dyang Dyanka Dyang | Tag Dha Gur Gur Dha ||",
    barLengthSteps: 16,
    shankhaTriggers: [
      { step: 0, durationMs: 2200, description: "Long sacred conch call welcoming Maa Durga" },
      { step: 12, durationMs: 1400, description: "Conch swell" }
    ],
    stepSequence: [
      { step: 0, stroke: "DHA", stick: "LEFT_HAND", velocity: 0.95, accent: true, kansor: "CLANG_HIGH", phonetic: "ড্যাং (Dyang)" },
      { step: 1, stroke: "NONE", stick: "NONE", velocity: 0.0, accent: false, kansor: "NONE", phonetic: "-" },
      { step: 2, stroke: "DYANG", stick: "RIGHT_KANCHI", velocity: 0.85, accent: false, kansor: "RING_LOW", phonetic: "ড্যাং (Dyang)" },
      { step: 3, stroke: "NONE", stick: "NONE", velocity: 0.0, accent: false, kansor: "NONE", phonetic: "-" },
      { step: 4, stroke: "DYANG", stick: "RIGHT_KANCHI", velocity: 0.90, accent: true, kansor: "CLANG_HIGH", phonetic: "ড্যাং (Dyang)" },
      { step: 5, stroke: "KUT", stick: "RIGHT_KANCHI", velocity: 0.50, accent: false, kansor: "NONE", phonetic: "কা (Ka)" },
      { step: 6, stroke: "DYANG", stick: "RIGHT_KANCHI", velocity: 0.88, accent: false, kansor: "RING_LOW", phonetic: "ড্যাং (Dyang)" },
      { step: 7, stroke: "NONE", stick: "NONE", velocity: 0.0, accent: false, kansor: "NONE", phonetic: "-" },
      { step: 8, stroke: "TA", stick: "RIGHT_KANCHI", velocity: 0.80, accent: false, kansor: "CLANG_HIGH", phonetic: "তাগ (Tag)" },
      { step: 9, stroke: "NONE", stick: "NONE", velocity: 0.0, accent: false, kansor: "NONE", phonetic: "-" },
      { step: 10, stroke: "DHA", stick: "LEFT_HAND", velocity: 0.98, accent: true, kansor: "RING_LOW", phonetic: "ধা (Dha)" },
      { step: 11, stroke: "NONE", stick: "NONE", velocity: 0.0, accent: false, kansor: "NONE", phonetic: "-" },
      { step: 12, stroke: "GUR_GUR", stick: "RIGHT_KANCHI", velocity: 0.70, accent: false, kansor: "CLANG_HIGH", phonetic: "গুড় (Gur)" },
      { step: 13, stroke: "GUR_GUR", stick: "RIGHT_KANCHI", velocity: 0.72, accent: false, kansor: "NONE", phonetic: "গুড় (Gur)" },
      { step: 14, stroke: "DHA", stick: "LEFT_HAND", velocity: 1.00, accent: true, kansor: "RING_LOW", phonetic: "ধা (Dha)" },
      { step: 15, stroke: "NONE", stick: "NONE", velocity: 0.0, accent: false, kansor: "NONE", phonetic: "-" }
    ]
  },
  {
    id: 3,
    partNumber: 3,
    num: "03",
    bengaliNum: "৩",
    title_bn: "৩. সন্ধ্যা আরতি ও ধুনুচি নৃত্য (\"তাক দুমা দুম তাক দুমা দুম, ঝম ঝমাকা ঝম\")",
    title_en: "3. Sandhya Aarti & Dhunuchi Dance Bol",
    tempoBpm: 124,
    style: "duk_kathi",
    styleLabel: "দুক কাঠি আরতি", styleLabelEnglish: "Duk-Kathi",
    taal: "Dhunuchi Kaharwa Drut Polyrhythm",
    timeSignature: "4/4",
    swingFactor: 0.16,
    description: "Electrifying rhythm played during the evening incense and Dhunuchi dance in the Mondol Bari Natmandir. Twin bamboo sticks create syncopated polyrhythms locking with Kanshor ringing.",
    vocalPhoneticBol: "তাক দুমা দুম | তাক দুমা দুম || ঝম ঝমাকা ঝম | ঝম ঝমাকা ঝম ||",
    romanizedBol: "Tak Duma Dum | Tak Duma Dum || Jham Jhamaka Jham | Jham Jhamaka Jham ||",
    barLengthSteps: 16,
    shankhaTriggers: [
      { step: 0, durationMs: 1600, description: "Powerful opening invocation call" },
      { step: 8, durationMs: 1600, description: "Second conch blast on mid-phrase" }
    ],
    stepSequence: [
      { step: 0, stroke: "TA", stick: "RIGHT_KANCHI", velocity: 0.90, accent: true, kansor: "CLANG_HIGH", phonetic: "তাক (Tak)" },
      { step: 1, stroke: "NONE", stick: "NONE", velocity: 0.0, accent: false, kansor: "NONE", phonetic: "-" },
      { step: 2, stroke: "DHA", stick: "LEFT_KANCHI", velocity: 0.95, accent: false, kansor: "RING_LOW", phonetic: "দু (Du)" },
      { step: 3, stroke: "KUT", stick: "RIGHT_KANCHI", velocity: 0.65, accent: false, kansor: "NONE", phonetic: "মা (Ma)" },
      { step: 4, stroke: "DHA", stick: "LEFT_KANCHI", velocity: 1.00, accent: true, kansor: "CLANG_HIGH", phonetic: "দুম (Dum)" },
      { step: 5, stroke: "NONE", stick: "NONE", velocity: 0.0, accent: false, kansor: "NONE", phonetic: "-" },
      { step: 6, stroke: "TA", stick: "RIGHT_KANCHI", velocity: 0.88, accent: false, kansor: "RING_LOW", phonetic: "তাক (Tak)" },
      { step: 7, stroke: "KUT", stick: "RIGHT_KANCHI", velocity: 0.60, accent: false, kansor: "NONE", phonetic: "মা (Ma)" },
      { step: 8, stroke: "DHA", stick: "LEFT_KANCHI", velocity: 1.00, accent: true, kansor: "CLANG_HIGH", phonetic: "দুম (Dum)" },
      { step: 9, stroke: "DYANG", stick: "RIGHT_KANCHI", velocity: 0.85, accent: false, kansor: "NONE", phonetic: "ঝম (Jham)" },
      { step: 10, stroke: "TA", stick: "RIGHT_KANCHI", velocity: 0.82, accent: false, kansor: "RING_LOW", phonetic: "ঝমা (Jhama)" },
      { step: 11, stroke: "KUT", stick: "LEFT_KANCHI", velocity: 0.68, accent: false, kansor: "CLANG_HIGH", phonetic: "কা (Ka)" },
      { step: 12, stroke: "DHA", stick: "LEFT_KANCHI", velocity: 0.95, accent: true, kansor: "RING_LOW", phonetic: "ঝম (Jham)" },
      { step: 13, stroke: "DYANG", stick: "RIGHT_KANCHI", velocity: 0.80, accent: false, kansor: "NONE", phonetic: "ঝম (Jham)" },
      { step: 14, stroke: "GUR_GUR", stick: "BOTH_KANCHI", velocity: 0.90, accent: true, kansor: "CLANG_HIGH", phonetic: "ঝমা (Jhama)" },
      { step: 15, stroke: "TA", stick: "RIGHT_KANCHI", velocity: 0.92, accent: false, kansor: "RING_LOW", phonetic: "ঝম (Jham)" }
    ]
  },
  {
    id: 4,
    partNumber: 4,
    num: "04",
    bengaliNum: "৪",
    title_bn: "৪. সন্ধিপূজা ও ১০৮ প্রদীপ মহোৎসব (\"কড়াতাং কড়াতাং গুড় গুড় ধা\")",
    title_en: "4. Sandhi Puja 108 Deepam Climax Bol",
    tempoBpm: 128,
    tempoClimaxBpm: 144,
    style: "accelerando",
    styleLabel: "মহোৎসব চরম লয়", styleLabelEnglish: "Accelerando",
    taal: "Sandhi Chhanda 16-Matra Accelerating Teental",
    timeSignature: "4/4",
    swingFactor: 0.12,
    description: "Played at the critical transition between Ashtami and Nabami as 108 lamps and blue lotuses are offered. Accelerates relentlessly with rolling thunder.",
    vocalPhoneticBol: "কড়াতাং কড়াতাং | গুড় গুড় ধা || তেরে কেটে তাক তাং | কড় কড় ধা ||",
    romanizedBol: "Karatan Karatan | Gur Gur Dha || Tere Kete Tak Tang | Kara Kara Dha ||",
    barLengthSteps: 16,
    shankhaTriggers: [
      { step: 0, durationMs: 2400, description: "Continuous resonant conch horn across Sandhi climax" },
      { step: 8, durationMs: 2200, description: "Second conch resonance" }
    ],
    stepSequence: [
      { step: 0, stroke: "KUT", stick: "LEFT_KANCHI", velocity: 0.75, accent: false, kansor: "CLANG_HIGH", phonetic: "কড়া (Kara)" },
      { step: 1, stroke: "DYANG", stick: "RIGHT_KANCHI", velocity: 0.95, accent: true, kansor: "NONE", phonetic: "তাং (Tang)" },
      { step: 2, stroke: "KUT", stick: "LEFT_KANCHI", velocity: 0.75, accent: false, kansor: "RING_LOW", phonetic: "কড়া (Kara)" },
      { step: 3, stroke: "DYANG", stick: "RIGHT_KANCHI", velocity: 0.95, accent: true, kansor: "NONE", phonetic: "তাং (Tang)" },
      { step: 4, stroke: "GUR_GUR", stick: "BOTH_KANCHI", velocity: 0.85, accent: false, kansor: "CLANG_HIGH", phonetic: "গুড় (Gur)" },
      { step: 5, stroke: "GUR_GUR", stick: "BOTH_KANCHI", velocity: 0.88, accent: false, kansor: "NONE", phonetic: "গুড় (Gur)" },
      { step: 6, stroke: "DHA", stick: "LEFT_KANCHI", velocity: 1.00, accent: true, kansor: "RING_LOW", phonetic: "ধা (Dha)" },
      { step: 7, stroke: "NONE", stick: "NONE", velocity: 0.0, accent: false, kansor: "NONE", phonetic: "-" },
      { step: 8, stroke: "TA", stick: "RIGHT_KANCHI", velocity: 0.85, accent: false, kansor: "CLANG_HIGH", phonetic: "তে (Te)" },
      { step: 9, stroke: "KUT", stick: "LEFT_KANCHI", velocity: 0.70, accent: false, kansor: "NONE", phonetic: "রে (Re)" },
      { step: 10, stroke: "TA", stick: "RIGHT_KANCHI", velocity: 0.88, accent: false, kansor: "RING_LOW", phonetic: "কে (Ke)" },
      { step: 11, stroke: "KUT", stick: "LEFT_KANCHI", velocity: 0.72, accent: false, kansor: "NONE", phonetic: "টে (Te)" },
      { step: 12, stroke: "TA", stick: "RIGHT_KANCHI", velocity: 0.92, accent: true, kansor: "CLANG_HIGH", phonetic: "তাক (Tak)" },
      { step: 13, stroke: "DYANG", stick: "RIGHT_KANCHI", velocity: 0.95, accent: true, kansor: "NONE", phonetic: "তাং (Tang)" },
      { step: 14, stroke: "GUR_GUR", stick: "BOTH_KANCHI", velocity: 0.95, accent: true, kansor: "CLANG_HIGH", phonetic: "কড়কড় (Kara-kara)" },
      { step: 15, stroke: "DHA", stick: "LEFT_KANCHI", velocity: 1.05, accent: true, kansor: "RING_LOW", phonetic: "ধা (Dha)" }
    ]
  },
  {
    id: 5,
    partNumber: 5,
    num: "05",
    bengaliNum: "৫",
    title_bn: "৫. বিসর্জন ও বিজয়া শোভাযাত্রা (\"বলো দুর্গা মাইকি জয়! আসছে বছর আবার হবে\")",
    title_en: "5. Bishorjan / Bisarjan Procession Bol",
    tempoBpm: 120,
    style: "bisarjan_swing",
    styleLabel: "বিসর্জন দোল", styleLabelEnglish: "Bisarjan Swing",
    taal: "Bisarjan Shobhajatra 8-Matra Syncopated Swing",
    timeSignature: "4/4",
    swingFactor: 0.20,
    description: "The deeply emotional, swaying groove played as Maa Durga is carried to the holy Ganges ghat for immersion. Heavy bass accents on 1 and 4, shimmering syncopated Kanshor.",
    vocalPhoneticBol: "বলো দুর্গা মাইকি জয় | আসছে বছর আবার হবে || তাং তাং কড়াতাং | ধা ধা গুড়গুড় ধা ||",
    romanizedBol: "Bolo Durga Maiki Joy | Aasche Bachhor Aabar Hobe || Tang Tang Karatang | Dha Dha Gurgur Dha ||",
    barLengthSteps: 16,
    shankhaTriggers: [
      { step: 0, durationMs: 2200, description: "Grand farewell conch salute" },
      { step: 8, durationMs: 1800, description: "Echoing immersion conch blast" }
    ],
    stepSequence: [
      { step: 0, stroke: "DYANG", stick: "RIGHT_KANCHI", velocity: 1.00, accent: true, kansor: "CLANG_HIGH", phonetic: "তাং (Tang)" },
      { step: 1, stroke: "NONE", stick: "NONE", velocity: 0.0, accent: false, kansor: "NONE", phonetic: "-" },
      { step: 2, stroke: "DYANG", stick: "RIGHT_KANCHI", velocity: 0.90, accent: false, kansor: "RING_LOW", phonetic: "তাং (Tang)" },
      { step: 3, stroke: "KUT", stick: "LEFT_KANCHI", velocity: 0.70, accent: false, kansor: "NONE", phonetic: "কড়া (Kara)" },
      { step: 4, stroke: "DYANG", stick: "RIGHT_KANCHI", velocity: 0.95, accent: true, kansor: "CLANG_HIGH", phonetic: "তাং (Tang)" },
      { step: 5, stroke: "NONE", stick: "NONE", velocity: 0.0, accent: false, kansor: "NONE", phonetic: "-" },
      { step: 6, stroke: "DHA", stick: "LEFT_KANCHI", velocity: 0.98, accent: true, kansor: "RING_LOW", phonetic: "ধা (Dha)" },
      { step: 7, stroke: "NONE", stick: "NONE", velocity: 0.0, accent: false, kansor: "NONE", phonetic: "-" },
      { step: 8, stroke: "DHA", stick: "LEFT_KANCHI", velocity: 1.00, accent: true, kansor: "CLANG_HIGH", phonetic: "ধা (Dha)" },
      { step: 9, stroke: "NONE", stick: "NONE", velocity: 0.0, accent: false, kansor: "NONE", phonetic: "-" },
      { step: 10, stroke: "GUR_GUR", stick: "BOTH_KANCHI", velocity: 0.88, accent: false, kansor: "RING_LOW", phonetic: "গুড় (Gur)" },
      { step: 11, stroke: "GUR_GUR", stick: "BOTH_KANCHI", velocity: 0.90, accent: false, kansor: "NONE", phonetic: "গুড় (Gur)" },
      { step: 12, stroke: "DHA", stick: "LEFT_KANCHI", velocity: 1.00, accent: true, kansor: "CLANG_HIGH", phonetic: "ধা (Dha)" },
      { step: 13, stroke: "DYANG", stick: "RIGHT_KANCHI", velocity: 0.85, accent: false, kansor: "NONE", phonetic: "তাং (Tang)" },
      { step: 14, stroke: "KUT", stick: "LEFT_KANCHI", velocity: 0.75, accent: false, kansor: "CLANG_HIGH", phonetic: "কা (Ka)" },
      { step: 15, stroke: "DHA", stick: "LEFT_KANCHI", velocity: 0.95, accent: false, kansor: "RING_LOW", phonetic: "ধা (Dha)" }
    ]
  },
  {
    id: 6,
    partNumber: 6,
    num: "06",
    bengaliNum: "৬",
    title_bn: "৬. এক কাঠি ও দুক কাঠি যুগলবন্দী (\"ধাগে তেটে তাক তাং, কড়াতাং কড়াতাং ধা\")",
    title_en: "6. Ak-Kathi & Duk-Kathi Master Solo Bol",
    tempoBpm: 138,
    style: "jugalbandi_solo",
    styleLabel: "যুগলবন্দী একক", styleLabelEnglish: "Jugalbandi Solo",
    taal: "Virtuoso Solo Jugalbandi Teental",
    timeSignature: "4/4",
    swingFactor: 0.10,
    description: "Virtuosic solo showing wrist damping, ghost notes, and lightning-fast bamboo stick snaps. A masterclass in authentic Bengali Dhak mastery.",
    vocalPhoneticBol: "ধাগে তেটে তাক তাং | কড়াতাং কড়াতাং ধা || তেরে কেটে তেরে কেটে | ঝম ঝমাকা ধা ||",
    romanizedBol: "Dhage Tete Tak Tang | Karatang Karatang Dha || Tere Kete Tere Kete | Jham Jhamaka Dha ||",
    barLengthSteps: 16,
    shankhaTriggers: [
      { step: 0, durationMs: 1800, description: "Solo opening invocation" },
      { step: 8, durationMs: 1600, description: "Mid solo blast" }
    ],
    stepSequence: [
      { step: 0, stroke: "DHA", stick: "LEFT_HAND", velocity: 1.05, accent: true, kansor: "CLANG_HIGH", phonetic: "ধা (Dha)" },
      { step: 1, stroke: "KUT", stick: "LEFT_HAND", velocity: 0.65, accent: false, kansor: "NONE", phonetic: "গে (Ge)" },
      { step: 2, stroke: "TA", stick: "RIGHT_KANCHI", velocity: 0.85, accent: false, kansor: "RING_LOW", phonetic: "তে (Te)" },
      { step: 3, stroke: "KUT", stick: "LEFT_KANCHI", velocity: 0.70, accent: false, kansor: "NONE", phonetic: "টে (Te)" },
      { step: 4, stroke: "TA", stick: "RIGHT_KANCHI", velocity: 0.95, accent: true, kansor: "CLANG_HIGH", phonetic: "তাক (Tak)" },
      { step: 5, stroke: "NONE", stick: "NONE", velocity: 0.0, accent: false, kansor: "NONE", phonetic: "-" },
      { step: 6, stroke: "DYANG", stick: "RIGHT_KANCHI", velocity: 1.00, accent: true, kansor: "RING_LOW", phonetic: "তাং (Tang)" },
      { step: 7, stroke: "NONE", stick: "NONE", velocity: 0.0, accent: false, kansor: "NONE", phonetic: "-" },
      { step: 8, stroke: "KUT", stick: "LEFT_KANCHI", velocity: 0.75, accent: false, kansor: "CLANG_HIGH", phonetic: "কড়া (Kara)" },
      { step: 9, stroke: "DYANG", stick: "RIGHT_KANCHI", velocity: 0.95, accent: true, kansor: "NONE", phonetic: "তাং (Tang)" },
      { step: 10, stroke: "KUT", stick: "LEFT_KANCHI", velocity: 0.75, accent: false, kansor: "RING_LOW", phonetic: "কড়া (Kara)" },
      { step: 11, stroke: "DYANG", stick: "RIGHT_KANCHI", velocity: 0.95, accent: true, kansor: "NONE", phonetic: "তাং (Tang)" },
      { step: 12, stroke: "GUR_GUR", stick: "BOTH_KANCHI", velocity: 0.92, accent: false, kansor: "CLANG_HIGH", phonetic: "তেরে (Tere)" },
      { step: 13, stroke: "GUR_GUR", stick: "BOTH_KANCHI", velocity: 0.95, accent: false, kansor: "NONE", phonetic: "কেটে (Kete)" },
      { step: 14, stroke: "DHA", stick: "LEFT_HAND", velocity: 1.10, accent: true, kansor: "RING_LOW", phonetic: "ঝম (Jham)" },
      { step: 15, stroke: "DYANG", stick: "RIGHT_KANCHI", velocity: 0.95, accent: true, kansor: "CLANG_HIGH", phonetic: "ধা (Dha)" }
    ]
  }
];

export const photoRiverRows = [
  {
    id: "river-row-1",
    direction: "scroll-left",
    speed: "normal",
    title: "প্রতিমা দর্শন ও নাটমন্দির আচার",
    photos: [
      {
        id: "river-101",
        title: "Traditional Ekchala Daker Saaj",
        title_bn: "সাবেকি একচালা ডাকের সাজ",
        category: "protima",
        categoryLabel: "প্রতিমা দর্শন",
        src: "/gallery/art_hero_idol.webp",
        desc_bn: "ফুরফুরা মন্ডল পরিবারের ১৯৯৭ সাল থেকে পূজিত সাবেকি একচালা প্রতিমার স্বর্ণাভ দিব্য দর্শন।",
        author: "মন্ডল পরিবার মহাফেজখানা",
        likes: 428
      },
      {
        id: "river-102",
        title: "Traditional Handcrafted Rice Flour Alpana",
        title_bn: "নাটমন্দির চত্বরে সাবেকি চালের গুঁড়োর আলপনা",
        category: "crafts",
        categoryLabel: "হস্তশিল্প ও আলপনা",
        src: "/gallery/user_alpona.webp",
        desc_bn: "শালুক, পদ্ম ও দেবী চরণের নিখুঁত হাতের টানে আঁকা মন্ডল বাড়ির ঐতিহ্যবাহী সাবেকি পিটুলি আলপনা।",
        author: "ফুরফুরা মণ্ডল পরিবার",
        likes: 412
      },
      {
        id: "river-103",
        title: "Kola Bou Nabapatrika Snan",
        title_bn: "নবপত্রিকা স্নান ও প্রবেশ",
        category: "protima",
        categoryLabel: "প্রতিমা দর্শন",
        src: "/gallery/art_dawn_mahalaya.webp",
        desc_bn: "সপ্তমীর ভোরে গঙ্গাজলে নবপত্রিকা স্নান করিয়ে নাটমন্দিরে প্রবেশ।",
        author: "ঐতিহ্য সংরক্ষণ পরিষদ",
        likes: 290
      },
      {
        id: "river-104",
        title: "Daker Saaj Ornaments Detail",
        title_bn: "শোলার ডাকের সাজ ও চালচিত্র",
        category: "protima",
        categoryLabel: "প্রতিমা দর্শন",
        src: "/gallery/art_heritage_emblem.webp",
        desc_bn: "প্রাচীন শিল্পীদের হাতে তৈরি নিখুঁত শোলার গহনা ও শিব-দুর্গা চালচিত্র।",
        author: "মন্ডল পরিবার সংগ্রহ",
        likes: 384
      },
      {
        id: "river-105",
        title: "Kumari Puja Sacred Grace",
        title_bn: "মহাষ্টমীর কুমারী পূজা",
        category: "protima",
        categoryLabel: "প্রতিমা দর্শন",
        src: "/gallery/art_afternoon_kash.webp",
        desc_bn: "মহাষ্টমীর পুণ্যলগ্নে জীবন্ত দেবীরূপে কুমারী কন্যার অর্চনা ও আরতি।",
        author: "ফুরফুরা মণ্ডল পরিবার",
        likes: 345
      },
      {
        id: "river-106",
        title: "Sandhya Aarti & Incense Smoke",
        title_bn: "ধুনোর ধোঁয়ায় সন্ধ্যা আরতি",
        category: "aarti",
        categoryLabel: "ধুনুচি ও আরতি",
        src: "/gallery/user_dhunuchi_01.webp",
        desc_bn: "নাটমন্দিরে ধুনোর সুবাসিত ধোঁয়া ও কাঁসর-ঘণ্টার বোলে সন্ধ্যার মহাশক্তি আরতি।",
        author: "ফুরফুরা মণ্ডল পরিবার",
        likes: 512
      }
    ]
  },
  {
    id: "river-row-2",
    direction: "scroll-right",
    speed: "normal",
    title: "ধুনুচি নাচ, ১০৮ প্রদীপ ও আরতি",
    photos: [
      {
        id: "river-201",
        title: "Dhunuchi Dance in Full Swing",
        title_bn: "ধুনুচি নৃত্যের উন্মাদনাময় মুহূর্ত",
        category: "aarti",
        categoryLabel: "ধুনুচি ও আরতি",
        src: "/gallery/user_dhunuchi_03.webp",
        desc_bn: "ঢাকের তাকডুম তাকডুম বোলে যুবক-বৃদ্ধদের একাত্ম হয়ে ধুনুচি নৃত্য।",
        author: "মন্ডল পরিবার মহাফেজখানা",
        likes: 462
      },
      {
        id: "river-202",
        title: "Aarti Fire Glow & Devotion",
        title_bn: "১০৮ প্রদীপ ও ধুনুচির অগ্নিশিখা",
        category: "aarti",
        categoryLabel: "ধুনুচি ও আরতি",
        src: "/gallery/user_dhunuchi_02.webp",
        desc_bn: "অষ্টমী-নবমীর মাহেন্দ্রক্ষণে ১০৮ প্রদীপের আলোর রোশনাইয়ে সন্ধিপূজা।",
        author: "ফুরফুরা মণ্ডল পরিবার",
        likes: 538
      },
      {
        id: "river-203",
        title: "Traditional Panchapradip Aarti",
        title_bn: "পঞ্চপ্রদীপ ও ধুনুচি আরতি বন্দনা",
        category: "aarti",
        categoryLabel: "ধুনুচি ও আরতি",
        src: "/gallery/user_dhunuchi_04.webp",
        desc_bn: "কূলপুরোহিতের পঞ্চপ্রদীপ ঘূর্ণন ও সমবেত ভক্তবৃন্দের পুষ্পাঞ্জলি নিবেদন।",
        author: "ঐতিহ্য স্মৃতি ভাণ্ডার",
        likes: 395
      },
      {
        id: "river-204",
        title: "Authentic Mondol Bari Alpana",
        title_bn: "নাটমন্দিরের সাবেকি চালের গুঁড়োর আলপনা",
        category: "crafts",
        categoryLabel: "হস্তশিল্প ও আলপনা",
        src: "/gallery/user_alpona.webp",
        desc_bn: "বংশপরম্পরায় সংরক্ষিত মন্ডল বাড়ির সাবেকি পিটুলি আলপনার মননশীল রূপ।",
        author: "ফুরফুরা মণ্ডল পরিবার",
        likes: 420
      },
      {
        id: "river-205",
        title: "Courtyard Dhunuchi Ecstasy",
        title_bn: "নাটমন্দিরে ধুনুচি আরতি ও উল্লাস",
        category: "aarti",
        categoryLabel: "ধুনুচি ও আরতি",
        src: "/gallery/user_dhunuchi_05.webp",
        desc_bn: "ধুনোর গন্ধ ও কাঁসর-ঘণ্টার বোলে মন্ডল বাড়ির প্রাঙ্গণে আন্তরিক উৎসবের আমেজ।",
        author: "মন্ডল পরিবার মহাফেজখানা",
        likes: 478
      },
      {
        id: "river-206",
        title: "Festive Joy & Farewell Dance",
        title_bn: "উৎসবের আনন্দ ও ধুনুচি অভিবাদন",
        category: "aarti",
        categoryLabel: "ধুনুচি ও আরতি",
        src: "/gallery/user_dhunuchi_06.webp",
        desc_bn: "নবমীর রাতে সমবেত আরতি ও ঢাকের বোলে দেবীকে শেষ অভিবাদন।",
        author: "ফুরফুরা মণ্ডল পরিবার",
        likes: 450
      }
    ]
  },
  {
    id: "river-row-3",
    direction: "scroll-left",
    speed: "normal",
    title: "ঐতিহ্য, সিঁদুর খেলা ও বিসর্জন",
    photos: [
      {
        id: "river-301",
        title: "Heritage Natmandir Courtyard",
        title_bn: "ঐতিহ্যবাহী নাটমন্দির প্রাঙ্গণ",
        category: "heritage",
        categoryLabel: "ঐতিহ্য ও স্মৃতি",
        src: "/gallery/user_dhunuchi_05.webp",
        desc_bn: "১৯৯৭ সাল থেকে সংরক্ষিত মন্ডল বাড়ির পূজার ঐতিহ্য ও পারিবারিক ইতিহাস।",
        author: "মন্ডল পরিবার মহাফেজখানা",
        likes: 334
      },
      {
        id: "river-302",
        title: "Sindoor Khela on Dashami",
        title_bn: "বিজয়া দশমীর সিঁদুর খেলা",
        category: "heritage",
        categoryLabel: "ঐতিহ্য ও স্মৃতি",
        src: "/gallery/art_morning_puja.webp",
        desc_bn: "দেবী বরণের পর গৃহবধূদের সিঁদুরে রাঙা আনন্দ ও চোখের জলে মা-কে বিদায়।",
        author: "ফুরফুরা মণ্ডল পরিবার",
        likes: 490
      },
      {
        id: "river-303",
        title: "Bisarjan Shobhajatra",
        title_bn: "বিসর্জন ও ভাসান শোভাযাত্রা",
        category: "heritage",
        categoryLabel: "ঐতিহ্য ও স্মৃতি",
        src: "/gallery/art_hero_idol.webp",
        desc_bn: "কাঁধে করে প্রতিমা নিয়ে ঘাটের দিকে যাত্রা — 'আসছে বছর আবার হবে'।",
        author: "ঐতিহ্য সংরক্ষণ পরিষদ",
        likes: 445
      },
      {
        id: "river-304",
        title: "Ancestral Gathering & Aarti",
        title_bn: "পরিবারের আন্তরিক মিলনমেলা ও আরতি",
        category: "heritage",
        categoryLabel: "ঐতিহ্য ও স্মৃতি",
        src: "/gallery/user_dhunuchi_04.webp",
        desc_bn: "দেশ-বিদেশে ছড়িয়ে থাকা পরিবারের সদস্যদের শিকড়ে ফেরার মধুর মুহূর্ত।",
        author: "মন্ডল পরিবার আর্কাইভ",
        likes: 362
      },
      {
        id: "river-305",
        title: "Mondol Bari Alpona Tradition",
        title_bn: "ঐতিহ্যবাহী আলপনা শিল্প",
        category: "crafts",
        categoryLabel: "হস্তশিল্প ও আলপনা",
        src: "/gallery/user_alpona.webp",
        desc_bn: "পুজো মণ্ডপ সাজাতে পরিবারের সদস্যদের নিপুণ হাতে আঁকা সাবেকি আলপনা।",
        author: "ফুরফুরা মণ্ডল পরিবার",
        likes: 380
      },
      {
        id: "river-306",
        title: "Shantijal & Subho Bijoya",
        title_bn: "শান্তিজল ও শুভ বিজয়ার আশীর্বাদ",
        category: "heritage",
        categoryLabel: "ঐতিহ্য ও স্মৃতি",
        src: "/gallery/art_midnight_serenity.webp",
        desc_bn: "বিসর্জনের পর অপরাজিতা সুতো ধারণ, শান্তিজল গ্রহণ ও গুরুজনদের প্রণাম।",
        author: "ফুরফুরা মণ্ডল পরিবার",
        likes: 512
      }
    ]
  }
];

export const onnotaCategories = [
  { id: "all", label: "সমস্ত সৃষ্টি", icon: "" },
  { id: "art", label: "চিত্রশিল্প ও অলঙ্করণ", icon: "" },
  { id: "photography", label: "উৎসব আলোকচিত্র", icon: "" },
  { id: "crafts", label: "হস্তশিল্প ও আলপনা", icon: "" },
  { id: "literature", label: "সাহিত্য ও স্মৃতিচারণ", icon: "" }
];

export const onnotaCreations = [
  {
    id: "on-01",
    title: "Sarada Rooprekha — Dashabhuja Mahamaya",
    title_bn: "শারদ রূপরেখা — দশভুজা মহামায়া",
    category: "art",
    categoryLabel: "চিত্রশিল্প ও অলঙ্করণ",
    src: "/onnota/art_hero_idol.webp",
    desc_bn: "জলরং ও ডিজিটাল ক্যানভাসে মা দুর্গার মহিষাসুরমর্দিনী রূপের পটচিত্র শৈলীর অলঙ্করণ।",
    desc_en: "Digital gouache and folk Patachitra fusion depicting Devi Durga's cosmic power.",
    author: "ভক্ত ও সুধীবৃন্দ", authorEnglish: "Devotee Community",
    date: "শরৎ ২০২৬",
    likes: 215,
    tag: "ডিজিটাল আর্ট"
  },
  {
    id: "on-02",
    title: "Sandhya Aarti Smoke & Incense Light",
    title_bn: "ধুনোর ধোঁয়ায় সন্ধ্যা আরতি — নাটমন্দির",
    category: "photography",
    categoryLabel: "উৎসব আলোকচিত্র",
    src: "/onnota/user_dhunuchi_01.webp",
    desc_bn: "সন্ধ্যা আরতির সময় ধুনোর মেঘ ও প্রদীপের আলোর জাদুকরী ফ্রেম।",
    desc_en: "Twilight reflections and sacred evening aarti lamps inside the heritage Natmandir.",
    author: "ভক্ত ও সুধীবৃন্দ", authorEnglish: "Devotee Community",
    date: "অক্টোবর ২০২৬",
    likes: 284,
    tag: "উৎসব ফটো"
  },
  {
    id: "on-03",
    title: "Traditional Handcrafted Rice Flour Alpana",
    title_bn: "নাটমন্দির চত্বরে সাবেকি চালের গুঁড়োর আলপনা",
    category: "crafts",
    categoryLabel: "হস্তশিল্প ও আলপনা",
    src: "/onnota/user_alpona.webp",
    desc_bn: "শালুক, পদ্ম ও দেবী চরণের নিখুঁত হাতের টানে আঁকা মন্ডল বাড়ির সাবেকি পিটুলি আলপনা।",
    desc_en: "Authentic handmade sacred floor art using rice flour paste with lotus flourishes.",
    author: "ভক্ত ও সুধীবৃন্দ", authorEnglish: "Devotee Community",
    date: "মহাষষ্ঠী ২০২৬",
    likes: 398,
    tag: "ঐতিহ্য আলপনা"
  },
  {
    id: "on-04",
    title: "Nostalgia of Autumn: Forgotten Puja Days",
    title_bn: "শিউলির গন্ধে শারদ স্মৃতি — পুজোর ফেলে আসা দিন",
    category: "literature",
    categoryLabel: "সাহিত্য ও স্মৃতিচারণ",
    src: "/onnota/art_dawn_mahalaya.webp",
    desc_bn: "ভোরের রেডিওতে মহালয়ার সুর, নতুন জামার গন্ধ ও শৈশবের পুজোর আন্তরিক স্মৃতিচারণ।",
    desc_en: "An evocative prose memoir reminiscing early morning AIR broadcasts and ancestral warmth.",
    author: "ভক্ত ও সুধীবৃন্দ", authorEnglish: "Devotee Community",
    date: "শরৎ সংকলন ২০২৬",
    likes: 165,
    tag: "স্মৃতি গদ্য"
  },
  {
    id: "on-05",
    title: "Trinayani — Divine Gaze of Compassion",
    title_bn: "ত্রিনয়নী — মায়ের শান্ত স্নেহময় রূপ",
    category: "art",
    categoryLabel: "চিত্রশিল্প ও অলঙ্করণ",
    src: "/onnota/art_night_grandeur.webp",
    desc_bn: "অ্যাক্রিলিক ক্যানভাসে ফুটে ওঠা মায়ের করুণাময়ী ত্রিনয়ন ও স্বর্ণাভ মুকুটের আভা।",
    desc_en: "Acrylic study focusing on the divine third eye radiating maternal benevolence.",
    author: "ভক্ত ও সুধীবৃন্দ", authorEnglish: "Devotee Community",
    date: "সেপ্টেম্বর ২০২৬",
    likes: 242,
    tag: "ক্যানভাস আর্ট"
  },
  {
    id: "on-06",
    title: "Autumn Cloudscapes over Furfura",
    title_bn: "কাশবনের দোলা ও শরতের নীল আকাশ",
    category: "photography",
    categoryLabel: "উৎসব আলোকচিত্র",
    src: "/onnota/art_afternoon_kash.webp",
    desc_bn: "ফুরফুরার দিগন্তজোড়া কাশবনে বাতাসের ঢেউ ও শুভ্র মেঘমালার শারদ অভিবাদন।",
    desc_en: "Dancing white Kash plumes framed against dramatic autumn cloudscapes.",
    author: "ভক্ত ও সুধীবৃন্দ", authorEnglish: "Devotee Community",
    date: "শরৎ ২০২৬",
    likes: 198,
    tag: "প্রকৃতি চিত্র"
  },
  {
    id: "on-07",
    title: "Panchapradip Aarti & Devotional Glow",
    title_bn: "পঞ্চপ্রদীপ আরতি ও মঙ্গলময় দীপ্তি",
    category: "photography",
    categoryLabel: "উৎসব আলোকচিত্র",
    src: "/onnota/user_dhunuchi_04.webp",
    desc_bn: "কূলপুরোহিতের পঞ্চপ্রদীপ ঘূর্ণন ও সমবেত ভক্তবৃন্দের পুষ্পাঞ্জলি নিবেদন।",
    desc_en: "Sacred brass lamps rotating in rhythmic circles during the grand evening stuti.",
    author: "ভক্ত ও সুধীবৃন্দ", authorEnglish: "Devotee Community",
    date: "মহাষ্টমী ২০২৬",
    likes: 326,
    tag: "উৎসব ফটো"
  },
  {
    id: "on-08",
    title: "Agomoni Ode — The Homecoming of Uma",
    title_bn: "আগমনী কবিতা — ঘরের মেয়ে মা আসছেন ঘরে",
    category: "literature",
    categoryLabel: "সাহিত্য ও স্মৃতিচারণ",
    src: "/onnota/art_dawn_mahalaya.webp",
    desc_bn: "\"মেঘের কোলে রোদ হেসেছে, বাদল গেছে টুটি... কৈলাস থেকে ঘরের মেয়ে ফিরছে মায়ের কুটির।\"",
    desc_en: "A poetic Bengali ode capturing the bittersweet emotions of daughter Uma's arrival.",
    author: "ভক্ত ও সুধীবৃন্দ", authorEnglish: "Devotee Community",
    date: "মহালয়া ২০২৬",
    likes: 310,
    tag: "শারদ কবিতা"
  },
  {
    id: "on-09",
    title: "Dhunuchi Dance Rhythm & Fury",
    title_bn: "ধুনুচির অগ্নিশিখা ও ছন্দের মেলবন্ধন",
    category: "photography",
    categoryLabel: "উৎসব আলোকচিত্র",
    src: "/onnota/user_dhunuchi_03.webp",
    desc_bn: "ধুনোর লালচে অঙ্গার ও ঢাকের উত্তাল তালে নাটমন্দিরে ধুনুচি নাচের আবেগঘন মুহূর্ত।",
    desc_en: "Authentic photographic capture of master dhunuchi dancers in ecstatic motion.",
    author: "ভক্ত ও সুধীবৃন্দ", authorEnglish: "Devotee Community",
    date: "মহানবমী ২০২৬",
    likes: 474,
    tag: "উৎসব ফটো"
  },
  {
    id: "on-10",
    title: "Festive Courtyard Celebrations",
    title_bn: "নাটমন্দিরে ধুনুচি আরতি ও উল্লাস",
    category: "photography",
    categoryLabel: "উৎসব আলোকচিত্র",
    src: "/onnota/user_dhunuchi_05.webp",
    desc_bn: "মন্ডল পরিবারের প্রাঙ্গণে ধুনুচি নাচ ও পরিবারের আন্তরিক উপস্থিতি।",
    desc_en: "Devotees and family gathered together during high-energy evening arati.",
    author: "ভক্ত ও সুধীবৃন্দ", authorEnglish: "Devotee Community",
    date: "বিজয়া ২০২৬",
    likes: 352,
    tag: "উৎসব ফটো"
  },
  {
    id: "on-11",
    title: "Ancestral Shola Crown Craft",
    title_bn: "সাবেকি শোলার মুকুট ও কদমমালা",
    category: "crafts",
    categoryLabel: "হস্তশিল্প ও আলপনা",
    src: "/onnota/art_heritage_emblem.webp",
    desc_bn: "বনেদি প্রতিমার জন্য তৈরি সূক্ষ্ম শোলার মুকুট, কদম ফুল ও ময়ূরপঙ্খী চালচিত্র।",
    desc_en: "Ancestral heritage craftsmanship sculpted purely out of aquatic shola reed.",
    author: "ভক্ত ও সুধীবৃন্দ", authorEnglish: "Devotee Community",
    date: "মহাসপ্তমী ২০২৬",
    likes: 189,
    tag: "শোলার কাজ"
  },
  {
    id: "on-12",
    title: "Joyous Festive Finale & Dhunuchi Dance",
    title_bn: "উৎসবের আনন্দ ও বিদায় অভিবাদন",
    category: "photography",
    categoryLabel: "উৎসব আলোকচিত্র",
    src: "/onnota/user_dhunuchi_06.webp",
    desc_bn: "ঢাকের চামড়ার প্রতিটি কম্পনে ধুনুচি হাতে দেবীকে প্রণতি জানানো ভক্তের নিখাদ ভক্তি।",
    desc_en: "Poignant final salute to Maa Durga before immersion with glowing incense embers.",
    author: "ভক্ত ও সুধীবৃন্দ", authorEnglish: "Devotee Community",
    date: "শরৎ ২০২৬",
    likes: 360,
    tag: "উৎসব ফটো"
  }
];
